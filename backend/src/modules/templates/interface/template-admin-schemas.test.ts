...
describe('templateReleaseBundleSchema', () => {
  ...
  it('uses a deterministic config hash', () => {
    expect(templateConfigHash({ b: 2, a: { d: 4, c: 3 } })).toBe(
      templateConfigHash({ a: { c: 3, d: 4 }, b: 2 }),
    )
  })
})
