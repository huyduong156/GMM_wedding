...
const source = (relativePath: string) =>
  readFileSync(path.join(frontendTemplates, relativePath), 'utf8')
...
describe('readSectionKeys', () => {
  ...
  it('supports sections mapped from a separately named key array', () => {
    expect(
      keys(
        readSectionKeys(
          "const keys = ['one', 'two']; export const config = { sections: keys.map((sectionKey) => ({ sectionKey })) }",
        ),
      ),
    ).toEqual(['one', 'two'])
  })
  ...
  it('supports direct string and tuple section arrays', () => {
    expect(keys(readSectionKeys("export const config = { sections: ['one', 'two'] }"))).toEqual([
      'one',
      'two',
    ])
    expect(
      keys(readSectionKeys("export const config = { sections: [['one', 'One'], ['two', 'Two']] }")),
    ).toEqual(['one', 'two'])
  })
})
