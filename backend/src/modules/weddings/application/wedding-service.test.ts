...
const event: WeddingEventView = {
  ...
  name: 'Lễ thành hôn',
  eventType: 'CEREMONY',
  startsAt: new Date('2026-12-12T10:00:00Z'),
  endsAt: null,
  timezone: 'Asia/Ho_Chi_Minh',
  venueName: null,
  addressLine: null,
...
function repository(): WeddingRepository {
  return {
    create: vi.fn().mockResolvedValue(wedding),
    listOwned: vi.fn().mockResolvedValue([wedding]),
    findOwned: vi.fn().mockResolvedValue(wedding),
    updateOwned: vi.fn().mockResolvedValue(wedding),
    softDeleteOwned: vi.fn().mockResolvedValue(true),
    listEventsOwned: vi.fn().mockResolvedValue([]),
    createEventOwned: vi.fn(),
    findEventOwned: vi.fn(),
    updateEventOwned: vi.fn(),
    deleteEventOwned: vi.fn(),
    dashboardOwned: vi.fn(),
    analyticsOwned: vi.fn(),
    listTemplates: vi.fn().mockResolvedValue([]),
    getTemplateVersion: vi.fn().mockResolvedValue(null),
    getContentOwned: vi.fn(),
    saveContentOwned: vi.fn(),
    publishOwned: vi.fn(),
    unpublishOwned: vi.fn(),
    slugAvailable: vi.fn(),
    getPublicSnapshot: vi.fn(),
    listWishesOwned: vi.fn(),
    moderateWishOwned: vi.fn(),
    promoteWishToGuest: vi.fn(),
    linkWishGuest: vi.fn(),
  }
}
...
describe('WeddingService', () => {
  ...
  it('maps optimistic concurrency failure to a stable conflict', async () => {
    ...
    await expect(
      new WeddingService(repo).update(actor, wedding.id, { revision: 1, name: 'Tên mới' }),
  ...
  it('rejects an event ending before it starts', async () => {
    const repo = repository()
    await expect(
      new WeddingService(repo).createEvent(actor, wedding.id, {
        name: 'Lễ thành hôn',
        eventType: 'CEREMONY',
        startsAt: new Date('2026-12-12T10:00:00Z'),
        endsAt: new Date('2026-12-12T09:00:00Z'),
        timezone: 'Asia/Ho_Chi_Minh',
        sortOrder: 0,
        isPublic: true,
      }),
  ...
  it('does not reveal a wedding owned by another user', async () => {
    ...
    await expect(new WeddingService(repo).get(actor, wedding.id)).rejects.toMatchObject({
  ...
  it('maps stale event updates to a stable conflict', async () => {
    ...
    await expect(
      new WeddingService(repo).updateEvent(actor, wedding.id, event.id, {
        revision: 1,
        name: 'Tiệc cưới',
      }),
  ...
  it('maps content revision conflicts and template validation errors', async () => {
    ...
    await expect(
      new WeddingService(repo).saveContent(actor, wedding.id, {
    ...
    await expect(
      new WeddingService(repo).saveContent(actor, wedding.id, {
        ...
        content: {},
        themeConfig: {},
        sectionConfig: { enabled: ['qr'], order: ['qr'] },
        revision: 1,
      }),
