...
export class WeddingService {
  ...
  create(actor: AuthenticatedUserActor, data: CreateWeddingData) {
    return this.repository.create(actor.userId, data)
  }
  list(actor: AuthenticatedUserActor) {
    return this.repository.listOwned(actor.userId)
  }
  async get(actor: AuthenticatedUserActor, weddingId: string) {
    return this.requireWedding(await this.repository.findOwned(actor.userId, weddingId))
  }
  async update(actor: AuthenticatedUserActor, weddingId: string, data: UpdateWeddingData) {
    ...
    if (wedding === 'conflict')
      throw new WeddingError(
        ...
        409,
        'Wedding was changed by another request',
      )
    return this.requireWedding(wedding)
  }
  async remove(actor: AuthenticatedUserActor, weddingId: string) {
    if (!(await this.repository.softDeleteOwned(actor.userId, weddingId)))
      throw new WeddingError('WEDDING_NOT_FOUND', 404, 'Wedding not found')
  }
  async listEvents(actor: AuthenticatedUserActor, weddingId: string) {
    return this.requireWedding(await this.repository.listEventsOwned(actor.userId, weddingId))
  }
  async createEvent(
    ...
    return this.requireWedding(
      await this.repository.createEventOwned(actor.userId, weddingId, data),
    )
  }
  async updateEvent(
    actor: AuthenticatedUserActor,
    ...
    if (event === 'conflict')
      throw new WeddingError(
        ...
        409,
        'Wedding event was changed by another request',
      )
  ...
  async dashboard(actor: AuthenticatedUserActor, weddingId: string, now = new Date()) {
    return this.requireWedding(await this.repository.dashboardOwned(actor.userId, weddingId, now))
  }
  ...
  async getContent(actor: AuthenticatedUserActor, weddingId: string, surface: WeddingSurfaceValue) {
    return this.requireWedding(
      await this.repository.getContentOwned(actor.userId, weddingId, surface),
    )
  }
  async saveContent(
    actor: AuthenticatedUserActor,
    ...
    if (result === 'conflict')
      throw new WeddingError(
        ...
        409,
        'Wedding content was changed by another request',
      )
    if (result === 'template-not-found')
      throw new WeddingError('WEDDING_TEMPLATE_NOT_FOUND', 404, 'Template version not found')
    if (result === 'template-incompatible')
      throw new WeddingError(
        'WEDDING_TEMPLATE_INCOMPATIBLE',
        400,
        'Template is not compatible with this publication surface',
      )
    if (result === 'section-invalid')
      throw new WeddingError(
        'WEDDING_SECTION_INVALID',
        400,
        'Section configuration is not supported by the selected template',
      )
    return result
  }
  async publish(actor: AuthenticatedUserActor, weddingId: string, data: PublishWeddingData) {
    ...
    if (result === 'not-ready')
      throw new WeddingError(
        ...
        400,
        'Wedding content and template are not ready to publish',
      )
    if (result === 'slug-taken')
      throw new WeddingError('WEDDING_SLUG_TAKEN', 409, 'Wedding slug is already in use')
    if (result === 'conflict')
      throw new WeddingError(
  ...
  async unpublish(actor: AuthenticatedUserActor, weddingId: string, surface: WeddingSurfaceValue) {
    if (!(await this.repository.unpublishOwned(actor.userId, weddingId, surface)))
      throw new WeddingError('WEDDING_NOT_FOUND', 404, 'Wedding not found')
  }
  slugAvailable(actor: AuthenticatedUserActor, slug: string, weddingId?: string) {
    return this.repository.slugAvailable(actor.userId, slug, weddingId)
  }
  async publicSnapshot(slug: string, surface: WeddingSurfaceValue) {
    ...
    if (!snapshot)
      throw new WeddingError('WEDDING_PUBLIC_NOT_FOUND', 404, 'Published wedding not found')
    return snapshot
  }
  async listWishes(actor: AuthenticatedUserActor, weddingId: string, filter: WishListFilter) {
    return this.requireWedding(
      await this.repository.listWishesOwned(actor.userId, weddingId, filter),
    )
  }
  async moderateWish(
    actor: AuthenticatedUserActor,
    ...
    const result = await this.repository.moderateWishOwned(
      actor.userId,
      weddingId,
      wishId,
      status,
    ...
    if (result === null) throw new WeddingError('WEDDING_NOT_FOUND', 404, 'Wedding not found')
    if (result === 'not-found')
      throw new WeddingError('WEDDING_WISH_NOT_FOUND', 404, 'Wish not found')
    return result
  }
  async promoteWishToGuest(
    ...
    weddingId: string,
    wishId: string,
    data: Parameters<WeddingRepository['promoteWishToGuest']>[3],
  ) {
    ...
    if (result === 'conflict')
      throw new WeddingError(
        ...
        409,
        'Wish is already linked to another guest',
      )
    return result
  }
  async linkWishGuest(
    ...
    if (result === null)
      throw new WeddingError('WEDDING_WISH_OR_GUEST_NOT_FOUND', 404, 'Wish or guest not found')
    if (result === 'conflict')
      throw new WeddingError(
        'WEDDING_WISH_ALREADY_LINKED',
        409,
