...
const roleWhere = (role: SystemRole, now: Date) => ({
  some: { role, revokedAt: null, OR: [{ expiresAt: null }, { expiresAt: { gt: now } }] },
})
...
type AdminUserRow = {
  ...
  createdAt: Date
  updatedAt: Date
  roles: Array<{ role: SystemRole; expiresAt: Date | null }>
  _count: { memberships: number; createdWeddings: number }
}
...
export class AdminUserService {
  ...
  async list(filter: ListFilter) {
    const now = new Date()
    const cursorRow = filter.cursor
      ? await this.db.user.findFirst({
          where: { id: filter.cursor, deletedAt: null },
    ...
    return {
      ...
      summary: { total, active, pendingVerification: pending, suspended },
    }
  }
  ...
  async get(userId: string) {
    const row = await this.db.user.findFirst({
      ...
      select: this.userSelect(new Date()),
    })
  ...
  async update(
    actorUserId: string,
    userId: string,
    input: { status?: UserStatus | undefined; roles?: SystemRole[] | undefined },
    requestId?: string,
  ) {
    if (actorUserId === userId)
      throw new AdminUserError(
        'SELF_ADMIN_USER_MUTATION_FORBIDDEN',
        409,
        'You cannot change your own admin account',
      )
    const current = await this.db.user.findFirst({
      ...
      select: {
        id: true,
        status: true,
        roles: { where: { revokedAt: null }, select: { role: true, expiresAt: true } },
      },
    })
    if (!current) throw new AdminUserError('USER_NOT_FOUND', 404, 'User not found')
    const now = new Date()
    const activeAdmin = current.roles.some(
      (role) => role.role === 'ADMIN' && (!role.expiresAt || role.expiresAt > now),
    )
    ...
    if (removingAdmin) {
      const adminCount = await this.db.user.count({
        where: { deletedAt: null, status: 'ACTIVE', roles: roleWhere('ADMIN', now) },
      })
      if (adminCount <= 1)
        throw new AdminUserError(
          'LAST_ADMIN_REQUIRED',
  ...
  async bulkStatus(actorUserId: string, userIds: string[], status: UserStatus, requestId?: string) {
    ...
    if (uniqueIds.includes(actorUserId))
      throw new AdminUserError(
        'SELF_ADMIN_USER_MUTATION_FORBIDDEN',
        409,
        'You cannot change your own admin account',
      )
    if (status !== 'SUSPENDED' && status !== 'ACTIVE' && status !== 'PENDING_VERIFICATION')
      throw new AdminUserError(
        'INVALID_USER_STATUS',
        400,
        'This user status cannot be set by admin',
      )
    if (status !== 'ACTIVE') {
      const now = new Date()
      const [adminCount, affectedAdminCount] = await Promise.all([
        ...
        this.db.user.count({
          where: {
            ...
            deletedAt: null,
            status: 'ACTIVE',
            roles: roleWhere('ADMIN', now),
          },
        }),
      ])
      if (adminCount - affectedAdminCount < 1)
        throw new AdminUserError(
          'LAST_ADMIN_REQUIRED',
          409,
          'At least one active platform administrator is required',
    ...
    const result = await this.db.$transaction(async (tx) => {
      ...
      await tx.auditLog.create({
        data: {
          actorUserId,
          action: 'admin.users_bulk_status_updated',
          resourceType: 'User',
          ...(requestId ? { requestId } : {}),
          metadata: { userIds: uniqueIds, status, updatedCount: updated.count },
        },
      })
  ...
  async audit(userId: string, limit: number, cursor?: string) {
    ...
    return {
      ...
      nextCursor: hasNext ? (logs[limit - 1]?.id ?? null) : null,
    }
  }
  ...
  private userSelect(now: Date): Prisma.UserSelect {
    return {
      ...
      _count: { select: { memberships: true, createdWeddings: true } },
    }
  }
  ...
  private toDto(row: AdminUserRow) {
    return {
      ...
      roles: row.roles.map((role: { role: SystemRole }) => role.role),
