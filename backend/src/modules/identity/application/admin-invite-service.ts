  ...
  async invite(
    ...
    const result = await this.db.$transaction(async (tx) => {
      ...
      const outbox = await tx.outboxEvent.create({
        data: {
          aggregateType: 'User',
          aggregateId: user.id,
          eventType: 'IdentityAdminInviteRequestedV1',
          payload: {
            userId: user.id,
    ...
    try {
      ...
      await this.db.outboxEvent.update({
        ...
        data: { status: 'COMPLETED', processedAt: new Date(), attempts: { increment: 1 } },
      })
    } catch {
      /* Keep the outbox pending for the delivery worker. */
    }
