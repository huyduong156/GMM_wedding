...
export const invitationQuerySchema = z.object({ guestId: uuid.optional(), status: z.enum(['ACTIVE', 'REVOKED']).optional(), limit: z.coerce.number().int().min(1).max(100).default(50), cursor: z.string().max(512).optional() })
export const updateInvitationSchema = z.object({ label: z.string().trim().min(1).max(160).nullable().optional(), maxPartySize: z.number().int().min(1).max(50).optional(), expiresAt: z.string().datetime({ offset: true }).transform((v) => new Date(v)).nullable().optional() }).strict().refine((value) => Object.keys(value).length > 0, 'At least one field is required')
export const bulkDeleteSchema = z.object({ ids: z.array(uuid).min(1).max(500).refine((ids) => new Set(ids).size === ids.length, 'IDs must be unique') }).strict()
