...
const categorySelect = {
  ...
  createdAt: true,
  updatedAt: true,
} satisfies Prisma.GuestCategorySelect
const groupSelect = {
  id: true,
  weddingId: true,
  name: true,
...
export class PrismaGuestRepository implements GuestRepository {
  ...
  private async validateReferences(
    weddingId: string,
    data: { categoryId?: string | null | undefined; groupId?: string | null | undefined },
  ) {
    if (
      data.categoryId !== undefined &&
      data.categoryId !== null &&
      !(await this.prisma.guestCategory.findFirst({
        where: { id: data.categoryId, weddingId, deletedAt: null },
        select: { id: true },
      }))
    )
      throw new GuestError('GUEST_CATEGORY_NOT_FOUND', 404, 'Guest category not found')
    if (
      data.groupId !== undefined &&
      data.groupId !== null &&
      !(await this.prisma.guestGroup.findFirst({
        where: { id: data.groupId, weddingId, deletedAt: null },
        select: { id: true },
      }))
    )
      throw new GuestError('GUEST_GROUP_NOT_FOUND', 404, 'Guest group not found')
  }
  async listOwned(
    userId: string,
    weddingId: string,
  ...
  async createOwned(userId: string, weddingId: string, data: CreateGuestData) {
    if (!(await this.owns(userId, weddingId))) return null
    await this.validateReferences(weddingId, data)
    try {
      return await this.prisma.guest.create({
        data: {
          ...
          ...(data.tableName !== undefined ? { tableName: data.tableName } : {}),
        },
        select: guestSelect,
      })
    } catch (error) {
  ...
  async updateOwned(userId: string, weddingId: string, guestId: string, data: UpdateGuestData) {
    if (!(await this.owns(userId, weddingId))) return null
    await this.validateReferences(weddingId, data)
    ...
    const result = await this.prisma.guest.updateMany({
      where: {
        ...
        weddingId,
        deletedAt: null,
        wedding: this.ownedWhere(userId, weddingId),
      },
      data: cleanData,
    })
    return result.count ? this.findOwned(userId, weddingId, guestId) : null
  }
  async deleteOwned(userId: string, weddingId: string, guestId: string) {
    if (!(await this.owns(userId, weddingId))) return null
    const result = await this.prisma.guest.updateMany({
      where: {
        id: guestId,
  ...
  async bulkDeleteOwned(userId: string, weddingId: string, guestIds: string[]) {
    if (!(await this.owns(userId, weddingId))) return null
    const result = await this.prisma.guest.updateMany({
      where: {
        id: { in: guestIds },
  ...
  async createCategory(userId: string, weddingId: string, data: CreateCategoryData) {
    ...
    let depth = 1
    if (data.parentId) {
      const parent = await this.prisma.guestCategory.findFirst({
        where: { id: data.parentId, weddingId, deletedAt: null },
        select: { depth: true },
      })
      if (!parent)
        throw new GuestError(
          'GUEST_CATEGORY_PARENT_NOT_FOUND',
          404,
          'Guest category parent not found',
        )
      if (parent.depth >= 3)
        throw new GuestError(
          'GUEST_CATEGORY_DEPTH_INVALID',
          400,
          'Guest category depth cannot exceed three levels',
        )
      depth = parent.depth + 1
    }
    return this.prisma.guestCategory.create({
      data: {
        weddingId,
        name: data.name,
        depth,
  ...
  async updateCategory(
    ...
    if (!(await this.owns(userId, weddingId))) return null
    const current = await this.prisma.guestCategory.findFirst({
      where: { id: categoryId, weddingId, deletedAt: null },
    ...
    if (data.parentId !== undefined) {
      if (data.parentId === categoryId)
        throw new GuestError('GUEST_CATEGORY_CYCLE', 400, 'A category cannot be its own parent')
      if (data.parentId === null) depth = 1
      else {
        const parent = await this.prisma.guestCategory.findFirst({
          where: { id: data.parentId, weddingId, deletedAt: null },
    ...
    const result = await this.prisma.guestCategory.updateMany({
      where: { id: categoryId, weddingId, deletedAt: null },
      data: {
    ...
    return result.count
      ? this.prisma.guestCategory.findUnique({ where: { id: categoryId }, select: categorySelect })
      : null
  }
  async deleteCategory(userId: string, weddingId: string, categoryId: string) {
    if (!(await this.owns(userId, weddingId))) return null
    const now = new Date()
  ...
  async bulkDeleteCategories(userId: string, weddingId: string, categoryIds: string[]) {
    if (!(await this.owns(userId, weddingId))) return null
    const now = new Date()
    return this.prisma.$transaction(async (tx) => {
      const result = await tx.guestCategory.updateMany({
  ...
  async createGroup(userId: string, weddingId: string, data: CreateGroupData) {
    if (!(await this.owns(userId, weddingId))) return null
    return this.prisma.guestGroup.create({
      data: { weddingId, name: data.name, ...(data.note !== undefined ? { note: data.note } : {}) },
      select: groupSelect,
    })
  }
  async updateGroup(userId: string, weddingId: string, groupId: string, data: UpdateGroupData) {
    if (!(await this.owns(userId, weddingId))) return null
    const result = await this.prisma.guestGroup.updateMany({
      where: { id: groupId, weddingId, deletedAt: null },
      data: {
        ...(data.name !== undefined ? { name: data.name } : {}),
        ...(data.note !== undefined ? { note: data.note } : {}),
      },
    ...
    return result.count
      ? this.prisma.guestGroup.findUnique({ where: { id: groupId }, select: groupSelect })
      : null
  }
  async deleteGroup(userId: string, weddingId: string, groupId: string) {
    if (!(await this.owns(userId, weddingId))) return null
    const result = await this.prisma.guestGroup.updateMany({
      where: { id: groupId, weddingId, deletedAt: null },
      data: { deletedAt: new Date() },
  ...
  async exportOwned(userId: string, weddingId: string): Promise<GuestExportRow[] | null> {
    ...
    return rows.map((row) => ({
      ...
      categoryPath: [
        row.category?.parent?.parent?.name,
  ...
  async importOwned(
    ...
    weddingId: string,
    rows: GuestImportRow[],
  ): Promise<GuestExportRow[] | null> {
    if (!(await this.owns(userId, weddingId))) return null
    return this.prisma.$transaction(async (tx) => {
      const result: GuestExportRow[] = []
      for (const row of rows) {
        let categoryId: string | undefined
        if (row.categoryPath) {
          const parts = row.categoryPath
            .split('/')
            .map((part) => part.trim())
            .filter(Boolean)
          if (parts.length > 3)
          ...
          for (const name of parts) {
            const existing = await tx.guestCategory.findFirst({
              where: { weddingId, name, parentId: parentId ?? null, deletedAt: null },
              select: { id: true, depth: true },
            })
            const category =
              existing ??
              (await tx.guestCategory.create({
                data: {
                  weddingId,
                  name,
                  depth: parentDepth + 1,
        ...
        if (row.groupName) {
          const group =
            (await tx.guestGroup.findFirst({
              where: { weddingId, name: row.groupName, deletedAt: null },
              select: { id: true },
            })) ??
            (await tx.guestGroup.create({
              data: { weddingId, name: row.groupName },
              select: { id: true },
            }))
          groupId = group.id
        }
        ...
        const data = {
          ...
          ...(row.tableName !== undefined ? { tableName: row.tableName } : {}),
        }
