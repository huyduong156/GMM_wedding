...
const wishSelection = z.object({
  wishId: z.string().uuid(),
  sortOrder: z.number().int().min(0).max(1000),
}).strict()

export const recapIdSchema = z.string().uuid()
export const recapSlugSchema = z.string().trim().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).min(3).max(64)
export const recapSaveSchema = z.object({
...
export const recapUnpublishSchema = z.object({
  revision: z.number().int().positive(),
}).strict()
