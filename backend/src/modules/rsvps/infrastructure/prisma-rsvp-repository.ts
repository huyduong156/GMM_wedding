...
export class PrismaRsvpRepository implements RsvpRepository {
  ...
  async promoteToGuest(userId:string,weddingId:string,rsvpId:string,data:{displayName?:string;categoryId?:string;groupId?:string}) {
    if (!(await this.owns(userId,weddingId))) return null
    return this.prisma.$transaction(async tx=>{ const response=await tx.rsvpResponse.findFirst({where:{id:rsvpId,weddingId},select:{guestId:true,partySize:true,anonymousGuestName:true}}); if(!response)return null; if(response.guestId){const guest=await tx.guest.findFirst({where:{id:response.guestId,weddingId},select:guestSelect});return guest?{guest}:null}; const name=data.displayName??response.anonymousGuestName;if(!name)throw new RsvpError("RSVP_GUEST_NAME_REQUIRED",400,"Guest name is required"); const guest=await tx.guest.create({data:{weddingId,slug:await newGuestSlug(name,async slug=>Boolean(await tx.guest.findFirst({where:{weddingId,slug},select:{id:true}}))),name,maxPartySize:Math.max(1,response.partySize),tags:[],...(data.categoryId?{categoryId:data.categoryId}:{}),...(data.groupId?{groupId:data.groupId}:{})},select:guestSelect});const linked=await tx.rsvpResponse.updateMany({where:{id:rsvpId,weddingId,guestId:null},data:{guestId:guest.id}});return linked.count?{guest}:"conflict" as const })
  }
