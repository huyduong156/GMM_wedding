...
const repository = (overrides: Partial<TaskRepository> = {}): TaskRepository => ({
  ...
  createOwned: vi.fn(),
  bulkCreateOwned: vi.fn(),
  updateOwned: vi.fn(),
  deleteOwned: vi.fn(),
  reorderOwned: vi.fn().mockResolvedValue({ updatedCount: 1 }),
  bulkStatusOwned: vi.fn().mockResolvedValue({ updatedCount: 1 }),
...
describe('TaskService', () => {
  it('maps revision conflicts', async () => {
    ...
    await expect(
      new TaskService(repo).update(actor, 'wedding-1', 'task-1', { status: 'DONE', revision: 1 }),
    ).rejects.toMatchObject({ code: 'TASK_REVISION_CONFLICT', status: 409 })
  })
  it('delegates bulk status to the owner-scoped repository', async () => {
    ...
    await expect(
      new TaskService(repo).bulkStatus(actor, 'wedding-1', ['task-1', 'task-2'], 'DONE'),
    ).resolves.toEqual({ updatedCount: 2 })
    expect(bulkStatusOwned).toHaveBeenCalledWith(
      'user-1',
      'wedding-1',
      ['task-1', 'task-2'],
      'DONE',
