...
function prismaMock(maxPartySize: number, upsert = vi.fn()) {
  return {
    invitation: {
      findFirst: vi
        .fn()
        .mockResolvedValue({ id: 'invitation-1', guestId: null, maxPartySize, expiresAt: null }),
      findUniqueOrThrow: vi.fn().mockResolvedValue({ weddingId: 'wedding-1', maxPartySize }),
    },
    publishedWeddingSnapshot: {
      findFirst: vi.fn().mockResolvedValue({ wedding: { id: 'wedding-1' } }),
    },
    rsvpResponse: { upsert },
...
describe('PublicInteractionService', () => {
  it('rejects an RSVP above the invitation party-size limit', async () => {
    ...
    await expect(
      new PublicInteractionService(prisma).submitPersonalRsvp('mai-duc', 'anh-an', {
  ...
  it('upserts a valid personalized RSVP by invitation business key', async () => {
    ...
    await new PublicInteractionService(prisma).submitPersonalRsvp('mai-duc', 'anh-an', {
      attendance: 'ATTENDING',
      partySize: 2,
    })
    expect(upsert).toHaveBeenCalledWith(
      expect.objectContaining({ where: { invitationId: 'invitation-1' } }),
    )
  })
})
