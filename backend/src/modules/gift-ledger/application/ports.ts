...
export type GiftLedgerView = { id: string; weddingId: string; guestId: string | null; guestDisplayNameSnapshot: string; giftType: GiftType; amountMinor: string | null; currency: string | null; goldWeight: string | null; goldUnit: string | null; goldType: string | null; giftDescription: string | null; receiveMethod: GiftReceiveMethod; receivedAt: Date; note: string | null; reciprocityStatus: ReciprocityStatus; returnedAt: Date | null; revision: number; createdAt: Date; updatedAt: Date; linkedGuest: { id: string; displayName: string } | null }
...
export interface GiftLedgerRepository {
  ...
  createOwned(userId: string, weddingId: string, data: CreateGiftData): Promise<GiftLedgerView | null>
  updateOwned(userId: string, weddingId: string, entryId: string, data: UpdateGiftData): Promise<GiftLedgerView | 'conflict' | null>
  deleteOwned(userId: string, weddingId: string, entryId: string): Promise<boolean | null>
  summaryOwned(userId: string, weddingId: string, filter: GiftDateFilter): Promise<GiftSummary | null>
  exportOwned(userId: string, weddingId: string, filter: GiftDateFilter): Promise<GiftLedgerView[] | null>
  promoteToGuest(userId: string, weddingId: string, entryId: string, displayName: string): Promise<GiftLedgerView | null>
  linkGuest(userId: string, weddingId: string, entryId: string, guestId: string): Promise<GiftLedgerView | null>
