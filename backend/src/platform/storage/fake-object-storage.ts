...
export class FakeObjectStorage implements ObjectStorage {
  private root() {
    return resolve(/* turbopackIgnore: true */ process.cwd(), getServerEnv().MEDIA_FAKE_ROOT)
  }
  ...
  async head(key: string): Promise<StoredObject | null> {
    try {
      ...
      return { sizeBytes: info.size, mimeType: 'application/octet-stream' }
    } catch {
      return null
    }
  }
  ...
  publicUrl(key: string) {
    const base = getServerEnv().MEDIA_PUBLIC_BASE_URL ?? '/api/media/fake-assets'
