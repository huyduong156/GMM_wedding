...
export async function GET(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { const { actor } = await requireAuthenticatedUser(request); const parsed = guestQuerySchema.parse(Object.fromEntries(request.nextUrl.searchParams)); return withApiHeaders(jsonResponse(await getGuestService().list(actor, await id(context), { query: parsed.q, categoryId: parsed.categoryId, groupId: parsed.groupId, limit: parsed.limit, cursor: parsed.cursor })), requestId) } catch (error) { return guestErrorResponse(error, requestId) } }
export async function POST(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); return withApiHeaders(jsonResponse({ guest: await getGuestService().create(actor, await id(context), await parseJson(request, createGuestSchema)) }, { status: 201 }), requestId) } catch (error) { return guestErrorResponse(error, requestId) } }
