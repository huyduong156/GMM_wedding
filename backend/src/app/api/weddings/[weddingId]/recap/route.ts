...
export async function GET(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse({ recap: await getRecapService().getDraft(actor.userId, weddingId) }),
      requestId,
    )
  } catch (error) {
...
export async function PUT(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const weddingId = weddingIdSchema.parse((await context.params).weddingId)
    return withApiHeaders(
      jsonResponse({ recap: await getRecapService().saveDraft(actor.userId, weddingId, input) }),
      requestId,
    )
  } catch (error) {
    return recapErrorResponse(error, requestId)
