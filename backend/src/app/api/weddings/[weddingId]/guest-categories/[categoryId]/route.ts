...
import { updateCategorySchema } from '@/modules/guests/interface/guest-schemas'
import { getRequestId, jsonResponse } from '@/shared/http/api-response'
import { weddingIdSchema } from '@/modules/weddings/interface/wedding-schemas'
export const OPTIONS = optionsResponse
type Context = { params: Promise<{ weddingId: string; categoryId: string }> }
export async function DELETE(request: NextRequest, context: Context) {
  ...
  try {
    ...
    await getGuestService().removeCategory(actor, weddingIdSchema.parse(p.weddingId), p.categoryId)
...
export async function PATCH(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse({
        category: await getGuestService().updateCategory(
          actor,
