...
export async function GET(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { const { actor } = await requireAuthenticatedUser(request); const params = await ids(context); return withAuthHeaders(jsonResponse({ task: await getTaskService().find(actor, params.weddingId, params.taskId) }), requestId) } catch (error) { return taskErrorResponse(error, requestId) } }
export async function PATCH(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); const params = await ids(context); return withAuthHeaders(jsonResponse({ task: await getTaskService().update(actor, params.weddingId, params.taskId, await parseJson(request, updateTaskSchema)) }), requestId) } catch (error) { return taskErrorResponse(error, requestId) } }
export async function DELETE(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); const params = await ids(context); await getTaskService().remove(actor, params.weddingId, params.taskId); return withAuthHeaders(new Response(null, { status: 204 }), requestId) } catch (error) { return taskErrorResponse(error, requestId) } }
