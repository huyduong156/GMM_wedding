...
async function ids(context: Context) {
  ...
  return {
    ...
    taskId: taskIdSchema.parse(params.taskId),
  }
}
export async function GET(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse({ task: await getTaskService().find(actor, params.weddingId, params.taskId) }),
      requestId,
    )
  } catch (error) {
...
export async function PATCH(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse({
        task: await getTaskService().update(
          actor,
          params.weddingId,
          params.taskId,
...
export async function DELETE(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const { actor } = await requireAuthenticatedUser(request)
    const params = await ids(context)
    await getTaskService().remove(actor, params.weddingId, params.taskId)
