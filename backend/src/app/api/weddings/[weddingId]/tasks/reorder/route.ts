...
export async function POST(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); const { taskIds } = await parseJson(request, reorderTaskSchema); return withApiHeaders(jsonResponse(await getTaskService().reorder(actor, weddingIdSchema.parse((await context.params).weddingId), taskIds)), requestId) } catch (error) { return taskErrorResponse(error, requestId) } }
