...
export async function POST(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); return withAuthHeaders(jsonResponse({ items: await getTaskService().applyTemplate(actor, weddingIdSchema.parse((await context.params).weddingId), await parseJson(request, applyTemplateSchema)) }, { status: 201 }), requestId) } catch (error) { return taskErrorResponse(error, requestId) } }
