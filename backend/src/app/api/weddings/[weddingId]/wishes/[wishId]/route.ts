...
export async function PATCH(request: NextRequest, context: Context) {
  ...
  try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); const params = await context.params; const input = await parseJson(request, wishModerationSchema); return withApiHeaders(jsonResponse({ wish: await getWeddingService().moderateWish(actor, weddingIdSchema.parse(params.weddingId), params.wishId, input.status, input.isPinned) }), requestId) } catch (error) { return weddingErrorResponse(error, requestId) }
}
