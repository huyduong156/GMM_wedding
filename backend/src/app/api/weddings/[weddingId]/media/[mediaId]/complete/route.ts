...
export async function POST(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); const p = await context.params; return withApiHeaders(jsonResponse(await getMediaManager().complete(actor.userId, weddingIdSchema.parse(p.weddingId), p.mediaId)), requestId) } catch (error) { return mediaErrorResponse(error, requestId) } }
