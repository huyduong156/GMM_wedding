...
export async function PUT(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request, { contentTypes: ['image/jpeg', 'image/png', 'image/webp'] }); const { actor } = await requireAuthenticatedUser(request); const p = await context.params; const body = new Uint8Array(await request.arrayBuffer()); return withApiHeaders(jsonResponse(await getMediaManager().uploadBytes(actor.userId, weddingIdSchema.parse(p.weddingId), p.mediaId, body, request.headers.get('content-type') ?? 'application/octet-stream')), requestId) } catch (error) { return mediaErrorResponse(error, requestId) } }
