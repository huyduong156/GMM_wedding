...
import { mediaIntentSchema, weddingIdSchema } from '@/modules/weddings/interface/wedding-schemas'
...
export async function GET(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse({
        items: await getMediaManager().list(
          actor.userId,
...
export async function POST(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const { actor } = await requireAuthenticatedUser(request)
    const input = await parseJson(request, mediaIntentSchema)
    return withApiHeaders(
      jsonResponse(
        await getMediaManager().createIntent(
          actor.userId,
          weddingIdSchema.parse((await context.params).weddingId),
          input,
        ),
