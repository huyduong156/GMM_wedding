...
export async function POST(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); const p = await context.params; return withAuthHeaders(jsonResponse({ entry: await getGiftLedgerService().unlink(actor, weddingIdSchema.parse(p.weddingId), giftEntryIdSchema.parse(p.entryId)) }), requestId) } catch (error) { return giftLedgerErrorResponse(error, requestId) } }
