...
export async function POST(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); const p = await context.params; const { guestId } = await parseJson(request, linkGuestSchema); return withApiHeaders(jsonResponse({ entry: await getGiftLedgerService().link(actor, weddingIdSchema.parse(p.weddingId), giftEntryIdSchema.parse(p.entryId), guestId) }), requestId) } catch (error) { return giftLedgerErrorResponse(error, requestId) } }
