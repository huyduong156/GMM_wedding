...
export async function POST(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const weddingId = weddingIdSchema.parse(p.weddingId)
    const entryId = giftEntryIdSchema.parse(p.entryId)
    const { displayName } = await parseJson(request, promoteGuestSchema)
    return withApiHeaders(
      jsonResponse(
        { entry: await getGiftLedgerService().promote(actor, weddingId, entryId, displayName) },
        { status: 201 },
      ),
      requestId,
    )
