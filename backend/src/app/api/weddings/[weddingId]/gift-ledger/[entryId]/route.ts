...
import {
  giftEntryIdSchema,
  updateGiftSchema,
} from '@/modules/gift-ledger/interface/gift-ledger-schemas'
...
async function ids(context: Context) {
  ...
  return {
    weddingId: weddingIdSchema.parse(p.weddingId),
    entryId: giftEntryIdSchema.parse(p.entryId),
  }
}
...
export async function GET(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse({ entry: await getGiftLedgerService().get(actor, p.weddingId, p.entryId) }),
      requestId,
    )
  } catch (error) {
...
export async function PATCH(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse({
        entry: await getGiftLedgerService().update(
          actor,
          p.weddingId,
          p.entryId,
          await parseJson(request, updateGiftSchema),
        ),
      }),
      requestId,
    )
...
export async function DELETE(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const { actor } = await requireAuthenticatedUser(request)
    const p = await ids(context)
    await getGiftLedgerService().remove(actor, p.weddingId, p.entryId)
