...
export async function GET(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { const { actor } = await requireAuthenticatedUser(request); const p = await ids(context); return withApiHeaders(jsonResponse({ entry: await getGiftLedgerService().get(actor, p.weddingId, p.entryId) }), requestId) } catch (error) { return giftLedgerErrorResponse(error, requestId) } }
export async function PATCH(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); const p = await ids(context); return withApiHeaders(jsonResponse({ entry: await getGiftLedgerService().update(actor, p.weddingId, p.entryId, await parseJson(request, updateGiftSchema)) }), requestId) } catch (error) { return giftLedgerErrorResponse(error, requestId) } }
export async function DELETE(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); const p = await ids(context); await getGiftLedgerService().remove(actor, p.weddingId, p.entryId); return withApiHeaders(new Response(null, { status: 204 }), requestId) } catch (error) { return giftLedgerErrorResponse(error, requestId) } }
