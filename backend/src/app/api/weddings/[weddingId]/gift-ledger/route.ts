...
export async function GET(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const query = giftQuerySchema.parse(Object.fromEntries(request.nextUrl.searchParams))
    return withApiHeaders(
      jsonResponse(await getGiftLedgerService().list(actor, await id(context), query)),
      requestId,
    )
  } catch (error) {
...
export async function POST(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse(
        {
          entry: await getGiftLedgerService().create(
            actor,
            await id(context),
            await parseJson(request, createGiftSchema),
          ),
        },
        { status: 201 },
      ),
