...
import { updateGroupSchema } from '@/modules/guests/interface/guest-schemas'
import { getRequestId, jsonResponse } from '@/shared/http/api-response'
import { weddingIdSchema } from '@/modules/weddings/interface/wedding-schemas'
export const OPTIONS = optionsResponse
type Context = { params: Promise<{ weddingId: string; groupId: string }> }
export async function PATCH(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse({
        group: await getGuestService().updateGroup(
          actor,
          weddingIdSchema.parse(p.weddingId),
          p.groupId,
          await parseJson(request, updateGroupSchema),
        ),
      }),
      requestId,
    )
...
export async function DELETE(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const { actor } = await requireAuthenticatedUser(request)
    const p = await context.params
    await getGuestService().removeGroup(actor, weddingIdSchema.parse(p.weddingId), p.groupId)
