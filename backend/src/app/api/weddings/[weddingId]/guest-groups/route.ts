...
import { groupSchema } from '@/modules/guests/interface/guest-schemas'
...
export async function GET(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse({ items: await getGuestService().groups(actor, await id(context)) }),
      requestId,
    )
  } catch (e) {
...
export async function POST(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse(
        {
          group: await getGuestService().createGroup(
            actor,
            await id(context),
            await parseJson(request, groupSchema),
          ),
        },
        { status: 201 },
      ),
