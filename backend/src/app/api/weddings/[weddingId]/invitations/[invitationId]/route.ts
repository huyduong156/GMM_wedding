...
export async function GET(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse({
        invitation: await getGuestService().getInvitation(
          actor,
          idsValue.weddingId,
          idsValue.invitationId,
        ),
      }),
      requestId,
    )
...
export async function PATCH(request: NextRequest, context: Context) {
  ...
  try {
    ...
    return withApiHeaders(
      jsonResponse({
        invitation: await getGuestService().updateInvitation(
          actor,
          idsValue.weddingId,
          idsValue.invitationId,
          await parseJson(request, updateInvitationSchema),
        ),
      }),
      requestId,
    )
