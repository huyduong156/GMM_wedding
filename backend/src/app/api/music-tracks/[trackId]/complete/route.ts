...
export async function POST(request: NextRequest, context: Context) {
  ...
  try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); return withAuthHeaders(jsonResponse(await getMusicService().complete(actor.userId, musicIdSchema.parse((await context.params).trackId), 'PERSONAL')), requestId) }
