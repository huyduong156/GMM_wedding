...
export async function POST(request: NextRequest) {
  ...
  try { assertSafeMutation(request); const { actor } = await requireAuthenticatedUser(request); const input = await parseJson(request, musicCreateSchema); return withApiHeaders(jsonResponse(await getMusicService().createPersonalIntent(actor.userId, input), { status: 201 }), requestId) }
