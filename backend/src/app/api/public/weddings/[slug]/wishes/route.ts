...
export async function POST(request: NextRequest, context: Context) {
  const requestId = getRequestId(request)
  try { assertSafeMutation(request); return withApiHeaders(jsonResponse(await getPublicInteractionService().submitWish((await context.params).slug, await parseJson(request, wishSchema)), { status: 201 }), requestId) } catch (error) { return guestErrorResponse(error, requestId) }
}
