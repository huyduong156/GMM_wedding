...
export async function POST(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requirePlatformAdmin(request); return withApiHeaders(jsonResponse(await getMusicService().activate(actor.userId, musicIdSchema.parse((await context.params).trackId))), requestId) } catch (error) { return musicErrorResponse(error, requestId) } }