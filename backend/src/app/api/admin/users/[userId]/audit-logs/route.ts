...
export async function GET(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { await requirePlatformAdmin(request); const { userId } = await context.params; const query = adminUserAuditQuerySchema.parse(Object.fromEntries(request.nextUrl.searchParams.entries())); return withApiHeaders(jsonResponse(await getAdminUserService().audit(adminUserIdSchema.parse(userId), query.limit, query.cursor)), requestId) } catch (error) { return adminUserErrorResponse(error, requestId) } }
