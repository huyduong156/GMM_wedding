...
export async function POST(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requirePlatformAdmin(request); const { userId } = await context.params; return withAuthHeaders(jsonResponse(await getAdminUserService().revokeSessions(actor.userId, adminUserIdSchema.parse(userId), requestId)), requestId) } catch (error) { return adminUserErrorResponse(error, requestId) } }
