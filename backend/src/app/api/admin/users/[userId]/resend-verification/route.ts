...
export async function POST(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const detail = await getAdminUserService().get(adminUserIdSchema.parse(userId))
    await getAuthService().resendVerification(detail.user.email, clientIp(request))
    return withApiHeaders(
      jsonResponse({ message: 'Verification request processed' }, { status: 202 }),
      requestId,
    )
  } catch (error) {
