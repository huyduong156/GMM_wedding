...
export async function POST(request: NextRequest, context: Context) { const requestId = getRequestId(request); try { assertSafeMutation(request); await requirePlatformAdmin(request); const { userId } = await context.params; const detail = await getAdminUserService().get(adminUserIdSchema.parse(userId)); await getAuthService().resendVerification(detail.user.email, clientIp(request)); return withAuthHeaders(jsonResponse({ message: 'Verification request processed' }, { status: 202 }), requestId) } catch (error) { return adminUserErrorResponse(error, requestId) } }
