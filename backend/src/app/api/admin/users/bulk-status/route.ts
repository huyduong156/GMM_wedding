...
export async function POST(request: NextRequest) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requirePlatformAdmin(request); const input = await parseJson(request, adminUserBulkStatusSchema); return withAuthHeaders(jsonResponse(await getAdminUserService().bulkStatus(actor.userId, input.userIds, input.status, requestId)), requestId) } catch (error) { return adminUserErrorResponse(error, requestId) } }
