...
export async function POST(request: NextRequest) { const requestId = getRequestId(request); try { assertSafeMutation(request); const { actor } = await requirePlatformAdmin(request); const input = await parseJson(request, adminUserInviteSchema); return withApiHeaders(jsonResponse(await getAdminInviteService().invite(actor.userId, input, requestId), { status: 202 }), requestId) } catch (error) { return adminUserErrorResponse(error, requestId) } }