...
describe('SlugService', () => {
  ...
  it('uses the preferred readable candidates before numeric suffixes', async () => {
    ...
    const taken = new Set(['huy-yen', 'huy-yen-wedding', 'wedding-huy-yen', 'huy-yen-web-wedding', 'web-wedding-huy-yen'])
  ...
  it('returns the first available preferred candidate', async () => {
    await expect(new SlugService().unique('Huy & Yến', async (slug) => slug === 'huy-yen')).resolves.toBe('huy-yen-wedding')
  })
})
