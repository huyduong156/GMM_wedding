...
async function capturedMessageFor(email: string, subject?: string, excludedToken?: string) {
  for (let attempt = 0; attempt < 40; attempt += 1) {
    ...
    const summaries = inbox.messages.filter(
      (message) =>
...
describe.sequential('authentication journey', () => {
  it('registers, verifies, logs in, authenticates, and logs out', async () => {
    ...
    const createdWedding = (await createWedding.json()) as {
      wedding: { id: string; revision: number }
    }
    firstOwnerWeddingId = createdWedding.wedding.id

    const listWeddings = await fetch(`${apiBase}/weddings`, {
      headers: { cookie: cookie as string },
    })
    expect(listWeddings.status).toBe(200)
    expect(
      ((await listWeddings.json()) as { items: Array<{ id: string }> }).items.some(
        (item) => item.id === firstOwnerWeddingId,
      ),
    ).toBe(true)
    ...
    expect(((await staleUpdate.json()) as { error: { code: string } }).error.code).toBe(
    ...
    const createEvent = await fetch(`${apiBase}/weddings/${firstOwnerWeddingId}/events`, {
      ...
      body: JSON.stringify({
        name: 'Lễ thành hôn',
        eventType: 'CEREMONY',
        startsAt: '2099-12-12T09:00:00+07:00',
    ...
    const dashboardBody = (await dashboard.json()) as {
      dashboard: {
        metrics: { guests: number; responses: number }
        nextEvent: { name: string }
        publication: { invitation: { views: null } }
      }
    }
    ...
    expect((await fetch(`${apiBase}/me`, { headers: { cookie: cookie as string } })).status).toBe(
      401,
    )
    expect(
      (
  ...
  it('separates the owner and platform-admin login surfaces', async () => {
    ...
    try {
      ...
      expect(
        (
          await fetch(`${apiBase}/weddings/${firstOwnerWeddingId}`, {
