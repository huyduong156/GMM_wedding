...
export function WeddingProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Không thể tải không gian cưới.')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const value = useMemo(
    () => ({
      ...
      addWedding: (wedding: Wedding) => {
        setWeddings((items) => [wedding, ...items])
        setActiveId(wedding.id)
      },
      replaceWedding: (wedding: Wedding) =>
        setWeddings((items) => items.map((item) => (item.id === wedding.id ? wedding : item))),
      removeWedding: (id: string) =>
        setWeddings((items) => {
