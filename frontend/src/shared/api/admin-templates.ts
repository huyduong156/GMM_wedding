...
export type AdminTemplateSourceStatus = 'DEVELOPMENT' | 'REVIEW' | 'READY' | 'DEPRECATED'
export type AdminTemplateVersion = {
  ...
  reviewStatus: AdminTemplateReviewStatus
  usageCount: number
  compatibility: {
    compatible: boolean
    issues: string[]
  ...
  recentAudit: Array<{
    ...
    action: string
    occurredAt: string
    actorUser: { displayName: string | null; email: string } | null
  }>
}
...
type LegacyAdminTemplate = Omit<AdminTemplate, 'versions'> & {
  versions: Array<
    Omit<AdminTemplateVersion, 'usageCount' | 'compatibility' | 'recentAudit'> &
      Partial<Pick<AdminTemplateVersion, 'usageCount' | 'compatibility' | 'recentAudit'>>
  >
}
...
export const adminTemplateApi = {
  async list(productType: AdminTemplate['productType'], styleKey?: string) {
    const result = await request<{ pendingReviewCount: number; items: LegacyAdminTemplate[] }>(
      '/admin/templates?productType=' +
        encodeURIComponent(productType) +
        (styleKey ? '&styleKey=' + encodeURIComponent(styleKey) : ''),
    )
    return {
      ...result,
      items: result.items.map((item): AdminTemplate => ({
        ...item,
        versions: item.versions.map((version): AdminTemplateVersion => ({
          ...version,
  ...
  detail(key: string, version: string) {
    return request<{ template: AdminTemplate & { version: AdminTemplateVersion } }>(
