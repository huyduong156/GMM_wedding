...
export function EditorPreviewModal({
  ...
  const deviceSwitch = (
    <div className="editor-preview-device-switch" aria-label="Chọn thiết bị xem trước">
      <button
        type="button"
  ...
  const surface = (
    <>
      ...
      <div className={`editor-iframe-shell is-${device}`}>
        <div className="editor-browser-bar">
          <i />
          <i />
          <i />
          <span>{device === 'desktop' ? 'Desktop · 1200 × 800' : 'Mobile · 550 × 950'}</span>
        </div>
        <div
          ref={viewportRef}
          className="editor-preview-viewport"
          style={{ aspectRatio: `${dimensions.width} / ${dimensions.height}` }}
        >
          {!ready ? (
            <div className="editor-preview-loading" role="status">
              <span aria-hidden="true" />
              <strong>Đang tải bản xem trước…</strong>
              <small>
                {templateKey ? `Đang chuẩn bị ${templateKey}` : 'Đang chuẩn bị giao diện'}
              </small>
            </div>
          ) : null}
          {switching ? (
            <div className="editor-preview-switching" role="status">
              <span aria-hidden="true" />
              <small>Đang chuyển chế độ xem…</small>
            </div>
          ) : null}
          <iframe
            ref={frameRef}
            title={`${title} ${templateKey ?? 'đang chọn'}`}
            src={route}
            onLoad={onLoad}
            style={frameStyle}
          />
  ...
  return (
    <main
      className={`editor-iframe-canvas ${open ? 'is-mobile-preview-open' : ''}`}
      aria-label={title}
      style={open ? undefined : draggable.style}
    ...
    >
      ...
      >
        ...
        <span>
          {open ? 'Thu nhỏ' : `Xem ${defaultDevice === 'desktop' ? 'desktop' : 'mobile'}`}
        </span>
      </button>
      {surface}
      <button
