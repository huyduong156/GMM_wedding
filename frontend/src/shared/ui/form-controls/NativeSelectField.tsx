...
export function NativeSelectField({ name: _name, id, value, defaultValue, onChange, children, disabled, required, className = '', 'aria-label': ariaLabel }: NativeSelectFieldProps) {
  const options = Children.toArray(children).filter(isValidElement).map((child) => { const props = child.props as SelectChildProps; return { value: props.value ?? String(props.children ?? ''), label: props.children, disabled: props.disabled } })
