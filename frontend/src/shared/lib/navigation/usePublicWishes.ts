...
const messageFor = (error: unknown) => error instanceof WeddingApiError ? error.message : 'Không thể gửi thông tin lúc này. Vui lòng thử lại.'

export function usePublicWishes({ weddingSlug, guestSlug, enabled = true }: Props): PublicWishesController {
  ...
  useEffect(() => {
    ...
    let active = true
    void weddingApi.publicWishes(weddingSlug).then((result) => { if (active) setItems(result.wishes) }).catch(() => {})
    return () => { active = false }
  }, [enabled, weddingSlug])
  const submit = useCallback(async (input: { guestName?: string; content: string }) => {
