...
export type LiveEditorUpdateMessage<T> = {
  type: typeof liveEditorEvents.update
  version: 1
  payload: T
}
export type LiveEditorScrollMessage<T extends string> = {
  type: typeof liveEditorEvents.scrollToSection
  version: 1
  payload: { sectionKey: T }
}
...
export function isLiveEditorUpdate<T>(value: unknown): value is LiveEditorUpdateMessage<T> {
  ...
  return (
    message.type === liveEditorEvents.update && message.version === 1 && Boolean(message.payload)
  )
}

export function isLiveEditorScroll<T extends string>(
  value: unknown,
): value is LiveEditorScrollMessage<T> {
  if (!value || typeof value !== 'object') return false
  const message = value as Partial<LiveEditorScrollMessage<T>>
  return (
    message.type === liveEditorEvents.scrollToSection &&
    message.version === 1 &&
    typeof message.payload?.sectionKey === 'string'
  )
}
...
export function useLiveEditorBridge<T, S extends string>(state: T) {
  ...
  const sendState = useCallback(() => {
    const message: LiveEditorUpdateMessage<T> = {
      type: liveEditorEvents.update,
      version: 1,
      payload: state,
    }
  ...
  const scrollToSection = useCallback((sectionKey: S) => {
    const message: LiveEditorScrollMessage<S> = {
      type: liveEditorEvents.scrollToSection,
      version: 1,
      payload: { sectionKey },
    }
    frameRef.current?.contentWindow?.postMessage(message, window.location.origin)
  ...
  useEffect(() => {
    const receiveReady = (event: MessageEvent) => {
      if (
        ...
        return
      if (event.data?.type === liveEditorEvents.ready) {
        setReady(true)
        setHydrated(false)
      }
...
export function useEditorSections<S extends string>(
  initial: S[],
  required: S[],
  canReorder: (section: S) => boolean = () => true,
) {
  const initialSectionRef = useRef(initial[0])
  const [selected, setSelected] = useState(initial[0])
  const [order, setOrder] = useState(initial)
  const [enabled, setEnabled] = useState(initial)
  const move = (key: S, step: -1 | 1) =>
    setOrder((current) => {
      const from = current.indexOf(key)
      const to = from + step
      if (to < 0 || to >= current.length || !canReorder(key) || !canReorder(current[to]))
        return current
      const next = [...current]
  ...
  const reset = useCallback((nextOrder: S[], nextEnabled: S[]) => {
    ...
    setSelected(nextOrder[0] ?? initialSectionRef.current)
  }, [])
  return { selected, setSelected, order, enabled, move, toggle, reset }
}

export async function readEditorImages(
  files: FileList | null,
  currentCount: number,
  maxItems = 12,
  ...
  const images = await Promise.all(
    accepted.slice(0, Math.max(0, maxItems - currentCount)).map(
      (file) =>
