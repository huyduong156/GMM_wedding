...
describe('Owner Workspace', () => {
  it('renders the public home page with working product links', () => {
    ...
    expect(screen.getByRole('link', { name: /Bắt đầu khám phá/i })).toHaveAttribute(
      'href',
      '#home-overview',
    )
    expect(
      screen.getByRole('heading', { name: 'Chuẩn bị ngày cưới, nhẹ nhàng hơn.' }),
    ).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /Bắt đầu tạo/i })).toHaveAttribute(
      'href',
      '/studio/invites/themes',
    )
    ...
    expect(
      screen.getByRole('heading', { name: 'Dễ dàng cho cả hai bạn và khách mời' }),
    ).toBeInTheDocument()
  ...
  it('renders the RSVP workspace and the expanded product navigation', () => {
    ...
    expect(screen.getByRole('link', { name: 'Kho thiệp' })).toHaveAttribute(
      'href',
      '/studio/invites/themes',
    )
    expect(screen.getByRole('link', { name: 'Kho website' })).toHaveAttribute(
      'href',
      '/studio/site/themes',
    )
  })
  ...
  it('renders the three-level guest category workspace', () => {
    ...
    expect(screen.getByRole('link', { name: 'Danh mục khách mời' })).toHaveAttribute(
      'aria-current',
      'page',
    )
  ...
  it('renders the theme gallery without the detailed editor', () => {
    ...
    expect(
      screen.getByRole('link', { name: 'Mở bản xem trước đầy đủ Élan d’Amour' }),
  ...
  it('renders the platform admin dashboard with its own navigation', () => {
    ...
    expect(screen.getByRole('link', { name: 'Kho thiệp online' })).toHaveAttribute(
      'href',
      '/gmm_admin/library/invites',
    )
    expect(screen.getByRole('link', { name: 'Kho website online' })).toHaveAttribute(
      'href',
      '/gmm_admin/library/websites',
    )
    expect(screen.getByRole('link', { name: 'Danh mục phong cách' })).toHaveAttribute(
      'href',
      '/gmm_admin/styles',
    )
  })
  ...
  it('renders the planning and post-wedding navigation placeholders', () => {
    ...
    expect(
      screen
        .getAllByRole('link', { name: 'Kho giao diện' })
        .some((link) => link.getAttribute('href') === '/studio/recap/themes'),
    ).toBe(true)
    expect(screen.getByRole('link', { name: 'Recap của bạn' })).toHaveAttribute(
      'href',
      '/studio/recap',
    )
  })
  ...
  it('renders the public modern luxe invitation with default content', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Một ngày, những khoảnh khắc đáng nhớ' }),
    ).toBeInTheDocument()
    ...
    expect(
      screen.getByRole('heading', { name: 'Gửi một lời chúc đến chúng mình' }),
    ).toBeInTheDocument()
    expect(screen.getByRole('region', { name: 'Thông tin hai bên gia đình' })).toBeInTheDocument()
    expect(
      screen.getByRole('heading', { name: 'Một ngày, những khoảnh khắc đáng nhớ' }),
  ...
  it('renders the Red Spider Lily Recap preview', () => {
  ...
  it('renders the Verdant Promise invitation with botanical interactions', () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: /Mở thiệp cưới của An Nhiên và Minh Khang/i }),
    )
      ...
      vi.fn().mockResolvedValue(
        new Response(
          JSON.stringify({
            items: [
              {
                ...
                phone: null,
                status: 'ACTIVE',
                emailVerifiedAt: '2026-07-18T08:00:00.000Z',
                lastLoginAt: '2026-07-18T08:00:00.000Z',
            ...
            nextCursor: null,
            summary: { total: 1, active: 1, pendingVerification: 0, suspended: 0 },
          }),
  ...
  it('renders the separate admin website library', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole('button', { name: 'Đồng bộ template' })).toBeInTheDocument(),
    )
