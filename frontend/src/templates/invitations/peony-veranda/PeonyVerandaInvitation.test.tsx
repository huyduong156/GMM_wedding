...
describe('PeonyVerandaInvitation', () => {
  ...
  it('hides RSVP choices after the guest responds', async () => {
    ...
    const interactions = { isPersonalized: true, guestName: 'Anh Ba Hưng', rsvp: { isPersonalized: true, submitting: false, submitted: false, error: '', submit }, wishes: { items: [], submitting: false, submitted: false, error: '', submit: vi.fn() } }
  ...
  it('uses the personalized guest identity when submitting a wish', async () => {
    const submit = vi.fn().mockResolvedValue(true)
    const interactions = { isPersonalized: true, guestName: 'Anh Ba Hưng', rsvp: { isPersonalized: true, submitting: false, submitted: false, error: '', submit: vi.fn() }, wishes: { items: [], submitting: false, submitted: false, error: '', submit } }
    render(<PeonyVerandaInvitation data={peonyVerandaFixture} interactions={interactions} />)
    expect(screen.queryByRole('textbox', { name: 'Tên của bạn' })).not.toBeInTheDocument()
    fireEvent.change(screen.getByRole('textbox', { name: 'Lời chúc' }), { target: { value: 'Chúc hai bạn trăm năm hạnh phúc.' } })
    fireEvent.click(screen.getByRole('button', { name: 'Gửi lời chúc' }))
    expect(submit).toHaveBeenCalledWith({ guestName: undefined, content: 'Chúc hai bạn trăm năm hạnh phúc.' })
    await waitFor(() => expect(screen.getByText('Cảm ơn bạn đã gửi lời chúc đến chúng mình.')).toBeInTheDocument())
  ...
  it('uses the entered name for a non-personalized wish', async () => {
    const submit = vi.fn().mockResolvedValue(true)
    const interactions = { isPersonalized: false, guestName: null, rsvp: { isPersonalized: false, submitting: false, submitted: false, error: '', submit: vi.fn() }, wishes: { items: [], submitting: false, submitted: false, error: '', submit } }
    render(<PeonyVerandaInvitation data={peonyVerandaFixture} interactions={interactions} />)
    fireEvent.change(screen.getByRole('textbox', { name: 'Tên của bạn' }), { target: { value: 'Ngọc Anh' } })
    fireEvent.change(screen.getByRole('textbox', { name: 'Lời chúc' }), { target: { value: 'Chúc hai bạn luôn vui vẻ.' } })
    fireEvent.click(screen.getByRole('button', { name: 'Gửi lời chúc' }))
    expect(submit).toHaveBeenCalledWith({ guestName: 'Ngọc Anh', content: 'Chúc hai bạn luôn vui vẻ.' })
  })
