...
const AURELIA_MOTION = { duration: 1.71, stagger: 0.06, listStagger: 0.5, ease: [0.22, 1, 0.36, 1] as const }
...
export function AureliaCourtRenderer({ data = aureliaCourtFixture, sectionConfig = aureliaCourtSectionConfig, editorMode = false, interactions }: Props) {
  ...
  useEffect(() => {
    ...
    void Promise.race([
      ...
      if (cancelled) return
      assetsReadyTimeout = window.setTimeout(() => { if (!cancelled) setAssetsReady(true) }, 500)
    })
    let assetsReadyTimeout: number | undefined
    return () => { cancelled = true; if (assetsReadyTimeout) window.clearTimeout(assetsReadyTimeout) }
  }, [data])
...
function Section({ sectionKey, className = '', style, children }: { sectionKey: string; className?: string; style?: React.CSSProperties; children: React.ReactNode }) {
  ...
  useEffect(() => {
    const section = scope.current as HTMLElement | null
    const elements = Array.from(section?.querySelectorAll<HTMLElement>('[data-motion-variant]') ?? [])
    if (!elements.length || !visible) return
    const animations = elements.map((element, index) => {
      const variant = element.dataset.motionVariant
      const listParent = element.parentElement
      const listIndex = listParent?.matches('.ac-timeline__list, .ac-countdown__grid, .ac-activities__track, .ac-activities__grid, .ac-gallery__controls, .ac-choice, .ac-families__grid')
        ? Array.from(listParent.children).indexOf(element)
        : -1
      const target = variant === 'fade-only'
        ? { opacity: 1 }
        : { opacity: 1, x: 0, y: 0, scale: 1 }
      return animate(element, target, {
        duration: reducedMotion ? 0 : AURELIA_MOTION.duration,
        delay: reducedMotion ? 0 : listIndex >= 0 ? listIndex * AURELIA_MOTION.listStagger : Math.min(index * AURELIA_MOTION.stagger, 0.44),
        ease: AURELIA_MOTION.ease,
      })
    })
    return () => animations.forEach((animation) => animation.stop())
  }, [animate, reducedMotion, scope, visible])
...
function Cover({ data }: { data: AureliaCourtData }) {
  return <Section sectionKey="cover" className="ac-cover"><img className="ac-royal-decor ac-royal-decor--pediment" data-motion-variant="fade-only" src={aureliaCourtArtwork.royalPediment} alt="" /><div className="ac-content ac-cover__plane"><p className="ac-kicker" data-motion-variant="fade-up">{data.cover.eyebrow}</p><h2 data-motion-variant="zoom-in">{data.couple.brideName}<span>&amp;</span>{data.couple.groomName}</h2>{data.cover.title !== data.couple.brideName + ' & ' + data.couple.groomName ? <p className="ac-lead" data-motion-variant="fade-up">{data.cover.title}</p> : null}<p className="ac-cover__message" data-motion-variant="fade-up">{data.cover.message}</p><p data-motion-variant="fade-up">{data.event.weddingDate} · {data.event.time}</p>{data.cover.heroMedia ? <img className="ac-user-media" data-motion-variant="fade-up" src={data.cover.heroMedia.src} alt={data.cover.heroMedia.alt} /> : null}</div></Section>
}

function Invitation({ data, guestName }: { data: AureliaCourtData; guestName: string }) {
  return <Section sectionKey="invitation" className="ac-letter"><img className="ac-floral ac-floral--letter" data-motion-variant="fade-only" src={aureliaCourtArtwork.sprig} alt="" /><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">Lời báo hỷ</p><h2 data-motion-variant="slide-up">{replaceGuest('Kính mời {guestName} đến chung vui', guestName)}</h2><p data-motion-variant="fade-up">{replaceGuest(data.invitation.message, guestName)}</p><img className="ac-divider" data-motion-variant="zoom-in" src={aureliaCourtArtwork.divider} alt="" /></div></Section>
}

function Families({ data }: { data: AureliaCourtData }) {
  const family = (side: typeof data.families.brideSide, kind: 'bride' | 'groom') => <div className={`ac-family ac-family--${kind}`} data-motion-variant="fade-up"><img className="ac-family-decor" data-motion-variant="fade-only" src={kind === 'bride' ? aureliaCourtArtwork.royalBust : aureliaCourtArtwork.royalBustMale} alt="" /><p className="ac-kicker" data-motion-variant="fade-up">{side.label}</p><div data-motion-variant="fade-up"><small>Ông bà</small><strong>{side.father}</strong><strong>{side.mother}</strong></div><p data-motion-variant="fade-up">{side.address}</p></div>
  return <Section sectionKey="families" className="ac-families"><img className="ac-floral ac-floral--family" data-motion-variant="fade-only" src={aureliaCourtArtwork.cascade} alt="" /><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">Hai bên gia đình</p><h2 data-motion-variant="slide-up">{data.families.title}</h2><p data-motion-variant="fade-up">{data.families.subtitle}</p><p className="ac-families__message" data-motion-variant="fade-up">{data.families.message}</p><div className="ac-families__grid" data-motion-variant="fade-up">{family(data.families.brideSide, 'bride')}<div className="ac-family-orbit" data-motion-variant="fade-only" aria-hidden="true"><span className="ac-family-orbit__ring" /><img src={aureliaCourtArtwork.crest} alt="" /></div>{family(data.families.groomSide, 'groom')}</div><img className="ac-divider" data-motion-variant="zoom-in" src={aureliaCourtArtwork.divider} alt="" /></div></Section>
}
...
function EventDetails({ data }: { data: AureliaCourtData }) {
  ...
  return <Section sectionKey="eventDetails" className="ac-event"><img className="ac-royal-decor ac-royal-decor--column ac-royal-decor--column-left" data-motion-variant="fade-only" src={aureliaCourtArtwork.royalColumn} alt="" /><img className="ac-royal-decor ac-royal-decor--column ac-royal-decor--column-right" data-motion-variant="fade-only" src={aureliaCourtArtwork.royalColumn} alt="" /><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">Thông tin ngày cưới</p><div className="ac-event__couple" data-motion-variant="fade-up"><div data-motion-variant="slide-left"><small>{data.eventDetails.brideTitle}</small><strong>{data.couple.brideName}</strong></div><div data-motion-variant="slide-right"><small>{data.eventDetails.groomTitle}</small><strong>{data.couple.groomName}</strong></div></div><p className="ac-event__ceremony" data-motion-variant="fade-up">{data.eventDetails.title}</p><p className="ac-event__time" data-motion-variant="fade-up">{data.eventDetails.message}</p>{date ? <div className="ac-date" data-motion-variant="zoom-in"><div className="ac-date__top"><span data-motion-variant="slide-left">{date.weekday}</span><strong data-motion-variant="zoom-in">{date.day}</strong><span data-motion-variant="slide-right">Tháng {date.month}</span></div><div className="ac-date__year">{date.year}</div>{lunar ? <small>(Nhằm ngày {lunar.day} tháng {lunar.month} năm {lunar.yearName} âm lịch)</small> : null}</div> : null}</div></Section>
}
...
function Countdown({ data }: { data: AureliaCourtData }) { const countdown = useWeddingCountdown(toCountdownTarget(data.event.weddingDate, data.event.time)); const values = [countdown.days, countdown.hours, countdown.minutes, countdown.seconds]; return <Section sectionKey="countdown" className="ac-countdown"><img className="ac-floral ac-floral--venue" data-motion-variant="fade-only" src={aureliaCourtArtwork.corner} alt="" /><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">Đếm ngược ngày vui</p><h2 data-motion-variant="slide-up">{data.event.weddingDate}</h2><div className="ac-countdown__calendar" data-motion-variant="zoom-in" aria-label={`Ngày cưới ${data.event.weddingDate}`}><span>Ngày vui</span><strong>{data.event.weddingDate}</strong><small>{data.event.time} · Hẹn gặp bạn trong ngày vui</small></div><div className="ac-countdown__grid">{['Ngày', 'Giờ', 'Phút', 'Giây'].map((label, index) => <strong data-motion-variant="fade-up" key={label}><b>{formatCountdownUnit(values[index])}</b><small>{label}</small></strong>)}</div></div></Section> }

function Timeline({ data }: { data: AureliaCourtData }) { const items = data.timeline.items ?? []; if (!items.length) return null; return <Section sectionKey="timeline" className="ac-timeline"><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">Lịch trình trong ngày</p><h2 data-motion-variant="slide-up">{data.timeline.title}</h2><p data-motion-variant="fade-up">{data.timeline.message}</p><div className="ac-timeline__list">{items.map((item) => <div className="ac-timeline__item" data-motion-variant="slide-left" key={`${item.time}-${item.title}`}><time data-motion-variant="fade-only">{item.time}</time><div><strong data-motion-variant="fade-up">{item.title}</strong><p data-motion-variant="fade-up">{item.detail}</p></div></div>)}</div></div></Section> }

function Venue({ data }: { data: AureliaCourtData }) { return <Section sectionKey="venue" className="ac-venue"><img className="ac-floral ac-floral--countdown" data-motion-variant="fade-only" src={aureliaCourtArtwork.cascade} alt="" /><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">{data.venue.title}</p><h2 data-motion-variant="slide-up">{data.venue.name}</h2><p data-motion-variant="fade-up">{data.venue.address}</p><p data-motion-variant="fade-up">{data.venue.message}</p><div className="ac-actions" data-motion-variant="fade-up"><a href={data.venue.mapUrl}>Mở bản đồ</a><a href={data.venue.calendarUrl}>Thêm vào lịch</a></div></div></Section> }

function Activities({ data }: { data: AureliaCourtData }) { const items = data.activities.items ?? []; if (!items.length) return null; const renderCard = (item: AureliaCourtData['activities']['items'][number], key: string) => <article data-motion-variant="fade-up" key={key}>{item.image ? <img data-motion-variant="zoom-in" src={item.image.src} alt={item.image.alt} /> : <img data-motion-variant="zoom-in" src={aureliaCourtArtwork.corner} alt="" />}<strong data-motion-variant="fade-up">{item.title}</strong></article>; const cards = items.length > 2 ? <div className="ac-activities__marquee" data-motion-variant="fade-up" aria-label="Các hoạt động trong ngày vui"><div className="ac-activities__track">{[...items, ...items].map((item, index) => renderCard(item, `${item.title}-${index}`))}</div></div> : <div className="ac-activities__grid" data-motion-variant="fade-up">{items.map((item) => renderCard(item, item.title))}</div>; return <Section sectionKey="activities" className="ac-activities"><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">Trong ngày vui</p><h2 data-motion-variant="slide-up">{data.activities.title}</h2><p data-motion-variant="fade-up">{data.activities.message}</p>{cards}</div></Section> }

function Gallery({ data }: { data: AureliaCourtData }) { const images = data.gallery.images ?? []; const [active, setActive] = useState(0); useEffect(() => { if (images.length < 2) return; const timer = window.setInterval(() => setActive((current) => (current + 1) % images.length), 4200); return () => window.clearInterval(timer); }, [images.length]); if (!images.length) return <Section sectionKey="gallery" className="ac-gallery"><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">Album ảnh</p><h2 data-motion-variant="slide-up">{data.gallery.title}</h2><p data-motion-variant="fade-up">{data.gallery.message}</p><div className="ac-empty" data-motion-variant="fade-up">Album sẽ được hiển thị tại đây.</div></div></Section>; return <Section sectionKey="gallery" className="ac-gallery"><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">Album ảnh</p><h2 data-motion-variant="slide-up">{data.gallery.title}</h2><p data-motion-variant="fade-up">{data.gallery.message}</p><div className="ac-gallery__carousel" data-motion-variant="fade-up" role="region" aria-label="Album ảnh cưới"><div className="ac-gallery__stage">{images.map((image, index) => { const rawOffset = (index - active + images.length) % images.length; const offset = rawOffset > images.length / 2 ? rawOffset - images.length : rawOffset; const visible = Math.abs(offset) <= 1; return <figure className="ac-gallery__card" key={image.src} aria-hidden={!visible} style={{ opacity: visible ? 1 : 0, transform: `translateX(calc(-50% + ${offset * 72}%)) translateZ(${offset === 0 ? 70 : 0}px) rotateY(${offset * -24}deg) scale(${offset === 0 ? 1 : .78})`, zIndex: offset === 0 ? 3 : 2 - Math.abs(offset) }}><img src={image.src} alt={image.alt} /></figure> })}</div><div className="ac-gallery__controls"><button data-motion-variant="slide-left" type="button" onClick={() => setActive((active - 1 + images.length) % images.length)} aria-label="Ảnh album trước">‹</button><span data-motion-variant="fade-up">{active + 1} / {images.length}</span><button data-motion-variant="slide-right" type="button" onClick={() => setActive((active + 1) % images.length)} aria-label="Ảnh album tiếp theo">›</button></div></div></div></Section> }
...
function ActionCard({ sectionKey, title, message, meta, action, interactions }: { sectionKey: string; title: string; message: string; meta?: string; action: string; interactions?: PublicInteractions }) {
  ...
  return <Section sectionKey={sectionKey} className={`ac-action ac-action--${sectionKey}`}><img className="ac-seal" data-motion-variant="zoom-in" src={aureliaCourtArtwork.seal} alt="" /><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">{isRsvp ? 'Xác nhận tham dự' : 'Lời chúc'}</p><h2 data-motion-variant="slide-up">{title}</h2><p data-motion-variant="fade-up">{message}</p>{meta ? <small data-motion-variant="fade-up">{meta}</small> : null}<form className="ac-action__form" data-motion-variant="fade-up" onSubmit={submit}>{isPersonalized ? <p className="ac-personalized" data-motion-variant="fade-up">Xác nhận cho {personalizedName || 'khách mời'}</p> : <label data-motion-variant="fade-up">Tên của bạn<input value={name} onChange={(event) => { setName(event.target.value); clearValidationError() }} autoComplete="name" /></label>}{isRsvp ? <div className="ac-choice" data-motion-variant="fade-up" aria-label="Lựa chọn tham dự"><button type="button" className={attendance === 'ATTENDING' ? 'is-selected' : ''} onClick={() => { setAttendance('ATTENDING'); clearValidationError() }}>Có thể tham dự</button><button type="button" className={attendance === 'DECLINED' ? 'is-selected' : ''} onClick={() => { setAttendance('DECLINED'); clearValidationError() }}>Xin phép vắng mặt</button></div> : <label data-motion-variant="fade-up">Lời chúc<textarea value={content} onChange={(event) => { setContent(event.target.value); clearValidationError() }} rows={3} placeholder="Viết lời chúc của bạn..." /></label>}<button className="ac-button" data-motion-variant="zoom-in" type="submit" disabled={disabled}>{submitted ? 'Đã gửi' : controller?.submitting ? 'Đang gửi...' : action}</button>{validationError || controller?.error ? <p data-motion-variant="fade-up" role="alert">{validationError || controller?.error}</p> : null}</form></div></Section>
}

function Gift({ data }: { data: AureliaCourtData }) { const [qrOpen, setQrOpen] = useState(false); const qrMedia = data.gift.qrMedia; const qrSrc = typeof qrMedia === 'string' ? qrMedia : qrMedia?.src; const hasQr = Boolean(qrSrc); const qrAlt = typeof qrMedia === 'object' && qrMedia?.alt ? qrMedia.alt : 'Mã QR mừng cưới'; return <Section sectionKey="gift" className="ac-gift"><div className="ac-gift__card"><img className="ac-gift__crest" data-motion-variant="zoom-in" src={aureliaCourtArtwork.crest} alt="" aria-hidden="true" /><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">Một lời chúc phúc</p><h2 data-motion-variant="slide-up">{data.gift.title}</h2><p className="ac-gift__message" data-motion-variant="fade-up">{data.gift.message}</p><div className="ac-gift__rule" aria-hidden="true"><span>✦</span></div><div className={`ac-gift__qr ${hasQr ? '' : 'ac-gift__qr--empty'} ${qrOpen ? 'is-open' : ''}`} data-motion-variant="zoom-in"><button className="ac-gift__qr-trigger" type="button" onClick={() => setQrOpen(true)} aria-expanded={qrOpen} tabIndex={qrOpen ? -1 : 0} aria-label="Mở mã QR mừng cưới">{hasQr ? <><img className="ac-qr ac-qr--preview" src={qrSrc} alt={qrAlt} /><span>Chạm để xem lớn hơn</span></> : <><img src={aureliaCourtArtwork.seal} alt="" aria-hidden="true" /><span>Chạm để xem mã QR</span></>}</button><div className="ac-gift__qr-panel" role="dialog" aria-label="Mã QR mừng cưới" aria-hidden={!qrOpen}>{hasQr ? <img className="ac-qr" src={qrSrc} alt={qrAlt} /> : <div className="ac-gift__qr-placeholder"><span aria-hidden="true" /><small>Mã QR sẽ hiển thị sau khi được thêm.</small></div>}<button className="ac-gift__qr-close" type="button" onClick={() => setQrOpen(false)} tabIndex={qrOpen ? 0 : -1} aria-label="Đóng mã QR">×</button></div></div><p className="ac-gift__thanks" data-motion-variant="fade-up">{data.gift.thankYouMessage}</p></div></div></Section> }

function Footer({ data }: { data: AureliaCourtData }) { const footerStyle = { '--ac-footer-image': data.footer.backgroundMedia ? `url("${data.footer.backgroundMedia.src}")` : 'none' } as React.CSSProperties; return <Section sectionKey="footer" className="ac-footer" style={footerStyle}><img className="ac-seal" data-motion-variant="zoom-in" src={aureliaCourtArtwork.seal} alt="" /><div className="ac-content"><p className="ac-kicker" data-motion-variant="fade-up">With love</p><h2 data-motion-variant="slide-up">{data.footer.title}</h2><p data-motion-variant="fade-up">{data.footer.message}</p><p className="ac-footer__names" data-motion-variant="fade-up">{data.couple.brideName} &amp; {data.couple.groomName}</p></div></Section> }
