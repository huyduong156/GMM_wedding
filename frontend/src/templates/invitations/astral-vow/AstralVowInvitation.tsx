...
export function AstralVowInvitation({ data, sectionConfig, editorMode = false, guestName, interactions }: { data?: AstralVowData; sectionConfig?: AstralVowSectionConfig; editorMode?: boolean; guestName?: string | null; interactions?: PublicInteractions }) {
  ...
  const showGuestNameInputs = !interactions?.isPersonalized && !connectedGuestName
  ...
  return <main ref={rootRef} className={`av-page ${opened ? 'is-opened' : ''} ${pageHidden ? 'is-page-hidden' : ''}`}><div className="av-stars" aria-hidden="true">{Array.from({ length: 22 }, (_, index) => <i key={index} style={{ '--x': `${index * 53 % 100}%`, '--y': `${index * 71 % 100}%`, '--delay': `${(index * 1.7) % 12}s` } as CSSProperties}/>)}</div>{active.has('music') && (content.music?.backgroundMusicUrl || editorMode) && <MusicPlayer ref={musicPlayerRef} src={content.music?.backgroundMusicUrl} title={content.music?.backgroundMusicName || 'Nhạc nền'} autoplay={content.music?.backgroundMusicAutoplay} active={opened && active.has('music')} editorMode={editorMode} />}{openingComplete && <FallingStars target=".av-page" className="av-page-stars" />}
    ...
    {(openingComplete || editorMode) && <div className="av-canvas"><div className="av-content">
      ...
      {(editorMode || timelineItems.length > 0) && <AstralSection active={active} order={order} sectionKey="timeline"><h2>{withGuest(content.timeline?.title ?? '', guest)}</h2><p>{withGuest(content.timeline?.message ?? '', guest)}</p>{timelineItems.length > 0 ? <div className="av-timeline">{timelineItems.map((item) => <article key={`${item.time}-${item.title}`}><i/><time>{withGuest(item.time, guest)}</time><h3>{withGuest(item.title, guest)}</h3><p>{withGuest(item.description, guest)}</p></article>)}</div> : <div className="av-timeline-empty">Thêm ít nhất một mốc thời gian để hiển thị lịch trình.</div>}</AstralSection>}
      ...
      <AstralSection active={active} order={order} sectionKey="rsvp" className="av-rsvp"><Sparkle size={22}/><h2>{withGuest(content.rsvp?.title ?? '', guest)}</h2><p>{withGuest(content.rsvp?.message ?? '', guest)}</p>{rsvpDone || interactions?.rsvp.submitted ? <strong>{withGuest(content.rsvp?.successMessage ?? '', guest)}</strong> : <>{showGuestNameInputs && <label className="av-guest-name-field"><span>Tên của bạn</span><input value={rsvpName} onChange={(event) => setRsvpName(event.target.value)} placeholder="Nhập tên để xác nhận tham dự" autoComplete="name" /></label>}<div><button type="button" className={choice === 'ATTENDING' ? 'is-picked' : ''} onClick={() => setChoice('ATTENDING')} disabled={interactions?.rsvp.submitting}>{content.rsvp?.attendingLabel}</button><button type="button" className={choice === 'DECLINED' ? 'is-picked' : ''} onClick={() => setChoice('DECLINED')} disabled={interactions?.rsvp.submitting}>{content.rsvp?.notAttendingLabel}</button></div><button type="button" onClick={submitRsvp} disabled={!choice || interactions?.rsvp.submitting}>{interactions?.rsvp.submitting ? 'Đang gửi…' : 'Xác nhận phản hồi'}</button>{(rsvpError || interactions?.rsvp.error) && <small role="alert">{rsvpError || interactions?.rsvp.error}</small>}</>}</AstralSection>
      <AstralSection active={active} order={order} sectionKey="guestbook" className="av-guestbook"><h2>{withGuest(content.guestbook?.title ?? '', guest)}</h2><p>{withGuest(content.guestbook?.message ?? '', guest)}</p>{wishDone || interactions?.wishes.submitted ? <strong>{withGuest(content.guestbook?.successMessage ?? '', guest)}</strong> : <>{showGuestNameInputs && <label className="av-guest-name-field"><span>Tên của bạn</span><input value={wishName} onChange={(event) => setWishName(event.target.value)} placeholder="Nhập tên để gửi lời chúc" autoComplete="name" /></label>}<textarea value={wish} onChange={(event) => setWish(event.target.value)} placeholder="Gửi đôi lời yêu thương…"/><button type="button" onClick={submitWish}>Gửi lời chúc</button>{(wishError || interactions?.wishes.error) && <small role="alert">{wishError || interactions?.wishes.error}</small>}</>}</AstralSection>
