# Vạn Hỷ — asset manifest and provenance

All files below are renderer-owned candidates for Phase 2.5. They are not user media and must not be exposed as editable fields in `template-config.ts`.
...
| `vh-double-happiness-v3.png` | Minimal flat paper-cut `囍` with one botanical accent | Generated with built-in `image_gen` on 2026-09-18 | OpenAI-generated project artwork; no external source image supplied | Preferred candidate; pending owner approval |
| `vh-paper-fan-v4.png` | Flat burgundy fan with gold line-art flowers and no surface ribs | Edited from `vh-paper-fan-v2.png` with built-in `image_gen` on 2026-09-18 | OpenAI-generated project artwork; no external source image supplied | Preferred candidate; pending owner approval |
| `vh-double-happiness-v4.png` | Flat burgundy `囍` with natural torn-paper edge | Edited from `vh-double-happiness-v3.png` with built-in `image_gen` on 2026-09-18 | OpenAI-generated project artwork; no external source image supplied | Preferred candidate; pending owner approval |

## Shared generation constraints

- Product/theme: `van-hy`, Vạn Hỷ, Vietnamese Gen Z Hỷ Sự invitation.
- Palette: `#5b0b18`, `#35050e`, `#781326`, `#a43a46`, `#c59645`.
- Medium: editorial botanical wedding-stationery illustration with subtle paper texture.
- Required: safe padding, no crop, no people, no text, no Chinese characters, no logo, no watermark, no PII, no external/licensed imagery.
- Intended canvas: mobile invitation capped at 480px; artwork must remain decorative and non-essential.

## File location

Final candidates are stored under:

`frontend/public/assets/images/templates/van-hy/decor/`

The original generated files remain in the built-in image-generation artifact directory for provenance. No external image URL is referenced by the template.
