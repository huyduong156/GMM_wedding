...
describe('ModernLuxeInvitation editor renderer', () => {
  it('opens immediately and applies enabled/order config without remounting', () => {
    const order: ModernLuxeSectionKey[] = [
      'cover',
      'gallery',
      'invitation',
      'eventDetails',
    ...
    render(
      <ModernLuxeInvitation
        editorMode
        data={{ brideName: 'An', groomName: 'Bình' }}
        sectionConfig={{ order, enabled: order.filter((key) => key !== 'gift') }}
      />,
    )
  ...
  it('keeps family content separate and supports explicit honorific fields', () => {
    render(
      <ModernLuxeInvitation
        editorMode
        data={{ groomFatherTitle: 'Bác', groomFather: 'Phạm Văn Minh' }}
      />,
    )
    ...
    expect(document.querySelector('[data-editor-section="invitation"]')).not.toHaveTextContent(
      'Phạm Văn Minh',
    )
  })
  ...
  it('keeps parsing only as a compatibility fallback for legacy family names', () => {
    ...
    expect(document.querySelector('[data-editor-section="families"]')).toHaveTextContent(
      'Phạm Văn Cũ',
    )
  })
})
