...
describe('GreenHydrangeaWebsite', () => {
  it('renders distinct couple portraits and the website contract', () => {
    ...
    const portraits = Array.from(container.querySelectorAll('.gh-person img')).map(image => image.getAttribute('src'))
  ...
  it('removes disabled optional sections', () => {
    const sectionConfig = { ...greenHydrangeaSections, enabled: greenHydrangeaSections.enabled.filter(key => key !== 'gallery') }
