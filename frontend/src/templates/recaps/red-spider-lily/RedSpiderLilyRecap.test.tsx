...
describe('Red Spider Lily recap contract', () => {
  it('renders every required section from the empty-media fixture', () => {
    ...
    const sections = [...document.querySelectorAll('[data-editor-section]')].map((section) =>
      section.getAttribute('data-editor-section'),
    )

    expect(sections).toEqual(
      expect.arrayContaining([
        ...redSpiderLilyRecapTemplateConfig.authoring.requiredSections,
  ...
  it('keeps neutral media placeholders and supports disabling optional sections', () => {
    ...
    const disabled = {
      ...
      optional: Object.fromEntries(
        Object.entries(redSpiderLilyRecapContent.optional).map(([key, value]) => [
          key,
          { ...value, enabled: false },
        ]),
      ) as typeof redSpiderLilyRecapContent.optional,
    }
  ...
  it('keeps the semantic composition order declared by the template', () => {
    ...
    expect(redSpiderLilyRecapTemplateConfig.composition.order).toEqual([
      'hero',
      'ourStory',
      'chapters',
      'moments',
  ...
  it('renders repeatable people and behind-the-scenes rails with item galleries', () => {
    ...
    expect(
      document.querySelectorAll('.rsl-behind-rail > .rsl-behind-item[class*="is-slot-"]'),
    ).toHaveLength(4)
    const galleryButtons = screen.getAllByRole('button', {
      name: /Xem khoanh khac|Mo album ky niem/i,
    })
  ...
  it('opens multi-image albums from moments and timeline covers', () => {
    ...
    fireEvent.click(
      document.querySelector('.rsl-3d-slide.is-active .rsl-moment-open') as HTMLElement,
    )
