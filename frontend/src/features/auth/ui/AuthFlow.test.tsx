...
describe('frontend authentication', () => {
  ...
  it('submits owner credentials and enters studio', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Mật khẩu'), {
      target: { value: 'Correct-Horse-Battery-42' },
    })
  ...
  it('redirects a missing admin session to admin login', async () => {
    ...
    renderApp('/gmm_admin')
    expect(
      screen.getByRole('status', { name: 'Đang kiểm tra phiên đăng nhập' }),
    ).toBeInTheDocument()
  ...
  it('checks the owner session once before loading the Wedding workspace', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation((input) =>
      Promise.resolve(
        new Response(JSON.stringify(String(input).endsWith('/me') ? { user } : { items: [] }), {
          status: 200,
          headers: { 'content-type': 'application/json' },
        }),
      ),
  ...
  it('resets the password from the email token', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Mật khẩu mới'), {
      target: { value: 'New-Correct-Horse-42' },
    })
