...
function GuestCategoriesContent({ activeWedding }: { activeWedding: Wedding | null }) {
  ...
  const create = async () => {
    ...
    try {
      ...
      closeDialog()
      await load()
      await alertSuccess('Danh mục đã được thêm vào cây.')
    } catch (cause) {
  ...
  const remove = async () => {
    ...
    const result = await notifications.fire({
      icon: 'warning',
      title: 'Xóa danh mục?',
      text: `${selected.length} danh mục sẽ được xóa. Danh mục con sẽ trở thành danh mục gốc.`,
    ...
    try {
      ...
      setSelected([])
      await load()
      await alertSuccess('Danh mục đã được xóa.')
    } catch (cause) {
      await alertError(cause instanceof Error ? cause.message : 'Không thể xóa danh mục.')
    } finally {
      setBusy(false)
    }
  }
  ...
  const render = (item: GuestCategory): JSX.Element => {
      ...
      <li key={item.id}>
        ...
        >
          <button
            ...
            aria-label={`${isOpen ? 'Thu gọn' : 'Mở rộng'} ${item.name}`}
          >
    ...
    <section className="categories-page" aria-labelledby="categories-heading">
      ...
      <div className="category-summary">
        ...
        <p>
          ...
          <span>Danh mục con được giữ lại khi xóa danh mục cha.</span>
        </p>
      </div>
      <div className="panel category-tree-panel">
        <header>
          ...
          {selected.length ? (
            ...
            <span>
              <UsersThree size={14} /> {roots.length} danh mục gốc
            </span>
          )}
        </header>
        {loading ? (
        ...
        >
          <section className="category-dialog" role="dialog" aria-modal="true">
            <header>
              ...
              <button type="button" onClick={closeDialog} aria-label="Đóng">
                <X size={18} />
              </button>
            </header>
