...
export function GuestCategoriesConnected() {
  const workspace = useOptionalWeddingWorkspace()
  if (!workspace) return <GuestCategoriesPage />
  return <GuestCategoriesConnectedContent activeWedding={workspace.activeWedding} />
}

function GuestCategoriesConnectedContent({ activeWedding }: { activeWedding: Wedding | null }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Không thể tải danh mục.')
    } finally {
      setLoading(false)
    }
  ...
  const create = async () => {
    ...
    setBusy(true)
    try {
      const result = await guestCategoryApi.create(activeWedding.id, {
        name: name.trim(),
        parentId: parent?.id ?? null,
      })
      setItems((current) => [...current, result.category])
      setParent(undefined)
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Không thể tạo danh mục.')
    } finally {
      setBusy(false)
    }
  }
  const remove = async () => {
    if (
      !activeWedding ||
      !selected.length ||
      !window.confirm(`Xóa ${selected.length} danh mục đã chọn?`)
    )
      return
    setBusy(true)
    try {
      await guestCategoryApi.removeMany(activeWedding.id, selected)
  ...
  const render = (item: GuestCategory) => {
    const nested = children(item.id)
    const isOpen = expanded.has(item.id)
    return (
      <li key={item.id}>
        <div className="category-row" style={{ '--category-depth': item.depth } as CSSProperties}>
          <button
          ...
          <div className="category-copy">
            ...
            <span>Cấp {item.depth}</span>
          </div>
          <span className="category-count">
            <UsersThree size={14} /> {item.guestCount ?? 0} khách
          </span>
          {item.depth < 3 ? (
            <button className="category-add-child" type="button" onClick={() => open(item)}>
              {' '}
              <Plus size={14} /> Thêm cấp con
            </button>
          ) : (
            <span className="category-limit">Cấp cuối</span>
          )}
          <button className="row-menu" type="button" aria-label={`Tùy chọn ${item.name}`}>
            <DotsThree size={18} weight="bold" />
          </button>
        </div>
        {nested.length && isOpen ? <ul>{nested.map(render)}</ul> : null}
      </li>
    )
  }
  return (
    <section className="categories-page" aria-labelledby="categories-heading">
      ...
      <div className="category-summary">
        ...
        <p>
          <strong>3 cấp tối đa</strong>
          <span>Danh mục được đồng bộ trực tiếp với danh sách khách mời.</span>
        </p>
      </div>
      <div className="panel category-tree-panel">
        <header>
          <div>
            <h2>Cây danh mục</h2>
            <p>Chọn nhiều danh mục để xóa khi nhập sai.</p>
          </div>
          {selected.length ? (
            <button
            ...
            <span>{roots.length} danh mục gốc</span>
          )}
        </header>
        {loading ? (
          <div className="guest-empty">Đang tải danh mục…</div>
        ) : error ? (
          ...
          <ul className="category-tree">{roots.map(render)}</ul>
        )}
      </div>
      {parent !== undefined && (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setParent(undefined)
          }}
        >
          <section className="category-dialog" role="dialog" aria-modal="true">
            <header>
              <div>
                ...
                <p>
                  {parent
                    ? `Nằm trong “${parent.name}” · Cấp ${parent.depth + 1}`
                    : 'Danh mục cấp 1'}
                </p>
              </div>
              <button type="button" onClick={() => setParent(undefined)} aria-label="Đóng">
                <X size={18} />
              </button>
            </header>
