      ...
      <li key={category.id}>
        ...
        >
          ...
          <span className="category-folder" aria-hidden="true">
            <FolderSimple size={18} weight={children.length && isExpanded ? 'fill' : 'regular'} />
          </span>
          <div className="category-copy">
            <strong>{category.name}</strong>
            <span>Cấp {category.depth}</span>
          </div>
          <span className="category-count">
            <UsersThree size={14} /> {category.guestCount} khách
          </span>
          {category.depth < 3 ? (
            <button
    ...
    <section className="categories-page" aria-labelledby="categories-heading">
      <header className="categories-heading">
        <div>
          <p className="breadcrumb">
            Mai & Đức <span>/</span> Khách mời <span>/</span> Danh mục
          </p>
          <h1 id="categories-heading">Danh mục khách mời</h1>
          <p>Tổ chức khách theo cây danh mục tối đa 3 cấp để lọc và gửi thiệp thuận tiện hơn.</p>
        </div>
      ...
      <div className="category-summary" aria-label="Tổng quan danh mục">
        ...
        <p>
          ...
          <span>Cấp 1: phía gia đình · Cấp 2: nhóm quan hệ · Cấp 3: nhánh chi tiết</span>
        </p>
      </div>
      ...
      <div className="panel category-tree-panel">
        <header>
          <div>
            <h2>Cây danh mục</h2>
            <p>Số khách ở danh mục cha bao gồm khách thuộc các danh mục con.</p>
          </div>
      ...
      {parentForNew !== undefined ? (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setParentForNew(undefined)
          }}
        >
          <section
          ...
          >
            <header>
              <div>
                <h2 id="new-category-title">
                  {parentForNew ? 'Thêm danh mục con' : 'Thêm danh mục gốc'}
                </h2>
                <p>
                  {parentForNew
                    ? `Nằm trong “${parentForNew.name}” · Cấp ${parentForNew.depth + 1}`
                    : 'Danh mục cấp 1'}
                </p>
              </div>
              <button type="button" onClick={() => setParentForNew(undefined)} aria-label="Đóng">
                <X size={18} />
              </button>
            </header>
            ...
            <input
              ...
              aria-describedby={error ? 'category-error' : 'category-helper'}
              onKeyDown={(event) => {
                if (event.key === 'Enter') createCategory()
                if (event.key === 'Escape') setParentForNew(undefined)
              }}
            />
            {error ? (
              ...
              <p className="field-helper" id="category-helper">
                Bạn có thể đổi tên hoặc di chuyển danh mục sau.
              </p>
            )}
            <footer>
              <button
                ...
                type="button"
                onClick={() => setParentForNew(undefined)}
              >
                Hủy
              </button>
