...
export function AdminLoginPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      navigate(adminRoutes.home, true)
    } catch (reason) {
      if (reason instanceof AuthApiError && reason.code === 'ADMIN_ACCESS_REQUIRED')
        setError('Tài khoản này không có quyền truy cập khu vực quản trị.')
      else if (reason instanceof AuthApiError && reason.code === 'INVALID_CREDENTIALS')
        setError('Email hoặc mật khẩu không chính xác.')
      else
        setError(reason instanceof Error ? reason.message : 'Không thể đăng nhập quản trị lúc này.')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <main className="admin-login-page">
      <section className="admin-login-intro" aria-label="Giới thiệu khu vực quản trị">
        ...
        <div className="admin-login-message">
          ...
          <span>
            Theo dõi người dùng, kiểm duyệt nội dung và xuất bản template từ một không gian làm việc
            bảo mật.
          </span>
        </div>
        <ul className="admin-login-assurances">
          <li>
            ...
            <span>
              ...
              <small>Mỗi thành viên chỉ thấy đúng nghiệp vụ được giao.</small>
            </span>
          </li>
          <li>
            <Fingerprint size={19} weight="duotone" />
            <span>
              <strong>Ghi nhận phiên truy cập</strong>
              <small>Các hoạt động quản trị quan trọng đều có thể truy vết.</small>
            </span>
          </li>
        </ul>
      </section>
        ...
        <div className="admin-login-form-wrap">
          ...
          <form className="admin-login-form" onSubmit={submit}>
            ...
            <button type="submit" className="admin-login-submit" disabled={submitting}>
              ...
              {!submitting ? <ArrowRight size={18} weight="bold" /> : null}
            </button>
          </form>
          <p className="admin-login-note">
            <ShieldCheck size={16} /> Chỉ tài khoản có quyền quản trị đang hoạt động mới được phép
            truy cập.
          </p>
          <AppLink to="/login" className="admin-login-back">
