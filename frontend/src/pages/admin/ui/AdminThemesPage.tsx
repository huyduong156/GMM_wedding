...
export function AdminThemesPage({ kind }: { kind: 'invitation' | 'website' }) {
  ...
  const visibleThemes = useMemo(
    () =>
      library.filter((theme) => {
        const matchesText = `${theme.name} ${theme.key} ${theme.style}`
          .toLowerCase()
    ...
    <div className="admin-dashboard admin-library-page">
      <header className="admin-page-heading">
        <div>
          ...
          <h1>Kho {isWebsite ? 'website online' : 'thiệp online'}</h1>
          <span>
            Kiểm duyệt, xuất bản và theo dõi vòng đời template {isWebsite ? 'website' : 'thiệp'}.
          </span>
        </div>
        <button className="button button-primary">
          <Plus size={17} weight="bold" /> Thêm template
        </button>
      </header>
        ...
        <div className="admin-library-toolbar">
          <label>
            ...
            <input
              ...
              placeholder="Tìm theo tên, mã hoặc phong cách"
            />
          </label>
          <div role="group" aria-label="Lọc trạng thái">
            {(
              ...
              >
                {label} <b>{value === 'all' ? library.length : count(value)}</b>
              </button>
            ))}
          </div>
        </div>
              ...
              <article className="admin-theme-card" key={theme.key}>
                <div className={`admin-theme-preview ${theme.preview}`}>
                  ...
                  <div className="admin-preview-canvas">
                    ...
                    <i />
                    <span>
                      {isWebsite ? 'Our story · Gallery · RSVP' : 'Save the date · 12.12.2026'}
                    </span>
                  </div>
                ...
                <footer>
                  {kind === 'invitation' &&
                  (theme.key === 'modern-luxe' ||
                    theme.key === 'verdant-promise' ||
                    theme.key === 'chibi-daydream') ? (
                    <AppLink
                      to={
                        theme.key === 'verdant-promise'
                          ? publicTemplateRoutes.verdantPromisePreview
                          : theme.key === 'chibi-daydream'
                            ? publicTemplateRoutes.chibiDaydreamPreview
                            : publicTemplateRoutes.modernLuxePreview
                      }
                      ariaLabel={`Xem trước ${theme.name}`}
                    >
