...
describe('InvitationEditorLivePage', () => {
  it('renders the live iframe editor, complete sections and image input', () => {
    render(
      <NavigationProvider>
        <InvitationEditorLivePage />
      </NavigationProvider>,
    )

    expect(screen.getByRole('heading', { name: /Chỉnh sửa thiệp/i })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'Quay lại kho giao diện' })).toHaveAttribute(
      'href',
      '/studio/invites/themes',
    )
    expect(screen.getByRole('button', { name: 'Lưu thiệp' })).toBeDisabled()
    expect(screen.getByRole('button', { name: 'Xem dạng điện thoại' })).toHaveAttribute(
      'aria-pressed',
      'true',
    )
  ...
  it('advises mobile users without blocking the editor', () => {
    ...
    expect(
      screen.getByRole('dialog', { name: 'Chỉnh thiệp dễ hơn trên máy tính' }),
    ).toBeInTheDocument()
    expect(screen.getByTitle(/Bản xem trước thiệp/)).toHaveStyle({
      width: '550px',
      height: '950px',
    })
  ...
  it('loads and saves the active invitation through the content API', async () => {
    const loadContent = vi.spyOn(weddingApi, 'content').mockResolvedValue({
      content: {
        content: { brideName: 'Lan', groomName: 'Minh' },
        schemaVersion: 1,
        revision: 4,
        surface: 'ONLINE_INVITATION',
        themeConfig: { palette: 'sage' },
        sectionConfig: {
          enabled: ['cover', 'invitation', 'families', 'eventDetails'],
          order: ['cover', 'invitation', 'families', 'eventDetails'],
        },
        templateVersion: {
    ...
    const save = vi.spyOn(weddingApi, 'saveContent').mockResolvedValue({
      content: {
        content: {},
        schemaVersion: 1,
        revision: 5,
    ...
    render(
      <NavigationProvider>
        <WeddingContext.Provider value={workspace}>
          <InvitationEditorLivePage />
    ...
    await waitFor(() =>
      expect(save).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          content: expect.objectContaining({ brideName: 'Lan Anh' }),
          themeConfig: { palette: 'sage' },
        }),
      ),
    )
  ...
  it('hydrates Peony Veranda sections and preview route for the live editor', async () => {
    vi.spyOn(weddingApi, 'content').mockResolvedValue({
      content: {
        content: {},
        schemaVersion: 1,
        revision: 2,
        surface: 'ONLINE_INVITATION',
        themeConfig: { palette: 'peony-veranda' },
        sectionConfig: { enabled: ['cover'], order: ['cover'] },
        templateVersion: {
          id: '44444444-4444-4444-8444-444444444444',
          key: 'peony-veranda',
          version: '1.0.0',
          config: { sections: ['cover'] },
        },
      },
    })
    render(
    ...
    expect(screen.getByTitle(/Bản xem trước thiệp/)).toHaveAttribute(
      'src',
      '/templates/invitations/peony-veranda/preview?editor=1',
    )
  })
})
