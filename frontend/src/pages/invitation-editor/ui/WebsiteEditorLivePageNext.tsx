...
type History = { data: Data; order: string[]; enabled: string[]; theme: Data }
type MediaTarget = { path: string; multiple: boolean; role: string; mediaValue?: 'url' | 'object' }
const initial = getWebsiteTemplate('editorial-vows')!
...
const keyOf = (value: unknown) =>
  typeof value === 'string'
    ...
    : typeof value === 'object' &&
      ...
      : ''
const sectionsOf = (key: string): Section[] =>
...
const mergeSections = (key: string, remote?: TemplateSectionConfig[]) => {
  const local = sectionsOf(key)
  const remoteKeys = new Map((remote ?? []).map((item) => [keyOf(item), item]))
  const source: Section[] = [
    ...local,
    ...(remote ?? [])
      .filter((item) => !local.some((x) => x.sectionKey === keyOf(item)))
      .map(
        (item) =>
          ({ sectionKey: keyOf(item), ...(item as unknown as Record<string, unknown>) }) as Section,
      ),
  ]
  return source.map((item) => {
    const remoteItem = remoteKeys.get(keyOf(item)) as unknown as Section | undefined
    return {
      ...item,
      ...(remoteItem ?? {}),
      sectionKey: keyOf(remoteItem) || keyOf(item),
      label: remoteItem?.label ?? item.label ?? keyOf(item),
    }
  })
}
const mergeData = (base: Data, stored: Data): Data => {
  ...
  Object.entries(stored ?? {}).forEach(([key, value]) => {
    next[key] =
      ...
      !Array.isArray(next[key])
        ? mergeData(next[key] as Data, value as Data)
        : value
  })
  return next
}
const setPath = (data: Data, path: string, value: unknown) => {
...
const defaults = (items: Section[], previous?: { order: string[]; enabled: string[] }) => {
  const keys = items.map((x) => x.sectionKey)
  const old = previous?.order.filter((x) => keys.includes(x)) ?? []
  const added = keys.filter((x) => !old.includes(x))
  return {
    order: [...old, ...added],
    enabled: [
      ...new Set([
        ...(previous?.enabled ?? []).filter((x) => keys.includes(x)),
        ...added,
        ...items.filter((x) => x.required).map((x) => x.sectionKey),
      ]),
    ],
  }
}
...
export function WebsiteEditorLivePageNext() {
  ...
  const slug = workspace?.activeWedding?.slug ?? ''
  const origin = typeof window === 'undefined' ? '' : window.location.origin
  const websiteUrl = slug ? `${origin}/${encodeURIComponent(slug)}/website` : ''
  ...
  const [data, setData] = useState<Data>(clone(initial.fixture))
  const [theme, setTheme] = useState<Data>({})
  const [definitions, setDefinitions] = useState<Section[]>(sectionsOf('editorial-vows'))
  ...
  const [pending, setPending] = useState<'website' | 'publish' | null>(null)
  ...
  const serverKeys = useRef<string[]>([])
  ...
  const required = useMemo(
    () => definitions.filter((x) => x.required).map((x) => x.sectionKey),
    [definitions],
  )
  const { order, enabled, reset, move, toggle } = useEditorSections(
    definitions.map((x) => x.sectionKey),
    required,
    (key) => definitions.find((x) => x.sectionKey === key)?.canReorder !== false,
  )
  const route = previewPath || getWebsiteTemplate(templateKey)?.previewPath || initial.previewPath
  const frame = useLiveEditorBridge<
    { data: Data; themeConfig: Data; sectionConfig: { enabled: string[]; order: string[] } },
    string
  >({ data, themeConfig: theme, sectionConfig: { enabled, order } })
  const signature = useCallback(
    (d: Data, o: string[], e: string[], t: Data) =>
      JSON.stringify({ data: d, order: o, enabled: e, theme: t }),
    [],
  )
  const load = useCallback(async () => {
    ...
    try {
      ...
      const defs = mergeSections(
        key,
        result.templateVersion.config.sections as TemplateSectionConfig[] | undefined,
      )
      const initialSections = defaults(defs, result.sectionConfig)
      const nextData = mergeData(
        clone(getWebsiteTemplate(key)?.fixture ?? initial.fixture),
        result.content,
      )
      setMissing(false)
      setTemplateKey(key)
      setPreviewPath(
        typeof result.templateVersion.config.previewPath === 'string'
          ...
          : (getWebsiteTemplate(key)?.previewPath ?? initial.previewPath),
      )
      setVersionId(result.templateVersion.id)
      setRevision(result.revision)
      setDefinitions(defs)
      setSelected(initialSections.order[0] ?? 'hero')
      serverKeys.current = ((result.templateVersion.config.sections as unknown[]) ?? [])
        .map(keyOf)
        .filter(Boolean)
      setData(nextData)
      setTheme(result.themeConfig ?? {})
      reset(initialSections.order, initialSections.enabled)
      baseline.current = signature(
        nextData,
        initialSections.order,
        initialSections.enabled,
        result.themeConfig ?? {},
      )
      setDirty(false)
    } catch (cause) {
  ...
  useEffect(() => {
    if (!mobileOpen) setDevice('desktop')
  }, [mobileOpen])
  const snapshot = () => ({
    data: clone(data),
    order: [...order],
  ...
  const save = async (): Promise<boolean> => {
    if (!weddingId || !versionId || revision === null) return false
    setSaving(true)
    setError('')
    try {
      const allowed = new Set(
        serverKeys.current.length ? serverKeys.current : definitions.map((x) => x.sectionKey),
      )
      const sectionConfig = {
        enabled: enabled.filter((x) => allowed.has(x)),
        order: order.filter((x) => allowed.has(x)),
      }
  ...
  const publish = async () => {
    ...
    setError('')
    try {
      const available = await weddingApi.slugAvailable(slug, weddingId)
      if (!available.available) {
        setError('Slug website không khả dụng.')
        return
      }
      ...
      await notifications.success('Đã công khai website')
  ...
  const unpublish = async () => {
    ...
    try {
      ...
      await notifications.success('Đã tạm đóng website')
      await load()
      await workspace?.refresh()
    } catch (cause) {
  ...
  const confirmMedia = (assets: MediaAsset[]) => {
    if (!mediaTarget || !assets.length) {
      setMediaOpen(false)
      return
    }
  ...
  const [published, setPublished] = useWebsitePublicationStatus(
    workspace?.activeWedding?.id ?? null,
    workspace?.activeWedding?.revision,
  )
  if (!workspace) return null
  if (loading && !versionId)
    return (
      <PageLoading
        label="Đang chuẩn bị trình chỉnh sửa website"
        detail="Nội dung website đang được mở ra."
      />
    )
  if (missing && !loading)
    return (
  ...
  return (
    <section className="invitation-editor website-editor">
        ...
        <div className="editor-toolbar-actions">
          ...
          <div className="editor-toolbar-public-actions">
            ...
            <button
              className="button button-secondary"
              type="button"
              disabled={!websiteUrl || saving}
              onClick={() => request('website')}
            >
              <Eye /> Xem website của tôi
            </button>
            <button
              className="button button-secondary"
              type="button"
              disabled={!versionId || saving || publishing}
              onClick={() => (published ? setUnpublishOpen(true) : request('publish'))}
            >
              {published ? (
                <>
                  <RocketLaunch /> Tạm đóng website
                </>
              ) : (
                <>
                  <RocketLaunch /> Công khai website
                </>
              )}
            </button>
            <button
              className="button button-secondary"
              type="button"
              disabled={!websiteUrl || saving || publishing}
        ...
        <aside className="editor-sections editor-section-accordion" aria-label="Cấu trúc website">
          <header>
            <div>
              ...
              <small>Mở từng phần để chỉnh sửa nội dung website</small>
            </div>
            {published ? (
              <div className="editor-structure-publication-status">
                <span>Website đang được công khai</span>
                {websiteUrl ? (
      ...
      {shareOpen && websiteUrl ? (
        <SharePublicationModal
          url={websiteUrl}
          surface="Website cưới"
      ...
      {pending ? (
        <ConfirmDialog
          ...
          text="Hãy lưu trước khi tiếp tục để website hiển thị dữ liệu mới nhất."
          onClose={() => setPending(null)}
          onConfirm={() => void continuePending()}
          confirmLabel="Lưu và tiếp tục"
          extra={
            pending === 'website' ? (
              <button
                className="button button-secondary"
      ...
      {unpublishOpen ? (
        <ConfirmDialog
          title="Tạm đóng website?"
          text="Khách truy cập sẽ không thể mở đường dẫn công khai. Nội dung đã lưu vẫn được giữ nguyên."
          onClose={() => setUnpublishOpen(false)}
          onConfirm={() => void unpublish()}
          confirmLabel={publishing ? 'Đang xử lý…' : 'Tạm đóng website'}
        />
      ) : null}
      {publishOpen ? (
        <ConfirmDialog
          url={websiteUrl}
          title="Công khai website"
          text={`Website sẽ dùng đường dẫn ${websiteUrl || 'chưa có slug'} để mọi người truy cập.`}
          onClose={() => setPublishOpen(false)}
          onConfirm={() => void publish()}
          confirmLabel={publishing ? 'Đang công khai…' : 'Công khai website'}
        />
      ) : null}
      {weddingId ? (
        <MediaManagerModal
...
function WebsiteSectionCard({
  ...
  return (
    ...
    >
      ...
      <div className={`editor-accordion-content ${expanded ? 'is-open' : ''}`}>
...
function ConfirmDialog({
  ...
  return (
    ...
    >
      <section className="editor-publish-dialog" role="dialog" aria-modal="true">
        <RocketLaunch size={28} />
        <h2>{title}</h2>
        <p>{text}</p>
        {url ? (
          <div className="editor-publish-url">
            <span className="editor-publish-url-label">Đường dẫn website</span>
            <a className="editor-publish-url-value" href={url} target="_blank" rel="noreferrer">
              {url}
            </a>
          </div>
        ) : null}
...
function useWebsitePublicationStatus(weddingId: string | null, revision?: number) {
  ...
  useEffect(() => {
    ...
    void weddingApi
      .dashboard(weddingId)
      .then((result) => {
        if (active) setPublished(result.dashboard.publication.website.published)
      })
      .catch(() => {
  ...
  return [published, setPublished] as const
}
