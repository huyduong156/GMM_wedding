...
export function resolveEditorSections(config: Record<string, unknown> | null | undefined, storedOrder: string[]): EditorSectionDefinition[] {
  ...
  return raw.map((item) => typeof item === 'string' ? { sectionKey: item } : item).filter((item) => item.sectionKey).map((item) => ({
    sectionKey: item.sectionKey,
    label: item.label ?? item.sectionKey,
    required: item.required ?? false,
    canToggle: item.canToggle ?? true,
    canReorder: item.canReorder ?? true,
    fields: Object.fromEntries(Object.entries(item.fields ?? {}).map(([key, field]) => [field.contentKey ?? key, { ...field, contentKey: field.contentKey ?? key }])),
  }))
}
