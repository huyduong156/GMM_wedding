...
export function InvitationEditorLivePage() {
  ...
  return <section className="invitation-editor" aria-labelledby="invitation-editor-heading">
    ...
    {activeWedding?.status === 'PUBLISHED' ? <div className="editor-published-bar"><span>Thiệp đang được công khai{publishSlug ? ` tại /${publishSlug}` : ''}.</span><button className="button button-secondary" type="button" disabled={publishing} onClick={() => setUnpublishOpen(true)}>{publishing ? 'Đang xử lý…' : 'Gỡ xuất bản'}</button></div> : null}
