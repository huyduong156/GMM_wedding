...
import { ArrowClockwise, ArrowDown, ArrowLeft, ArrowUUpLeft, ArrowUUpRight, ArrowUp, ArrowsOut, CaretDown, CheckCircle, Desktop, DeviceMobile, Eye, FloppyDisk, Image, MusicNote, Plus, RocketLaunch, ShareNetwork, Trash, X } from '@phosphor-icons/react'
...
export function InvitationEditorLivePage() {
  ...
  const openFullPreview = () => { if (!templateKey) return; sessionStorage.setItem(`gmm-invitation-preview:${templateKey}`, JSON.stringify({ data, palette, sectionConfig: { enabled, order } })); window.open(previewPath, '_blank', 'noopener,noreferrer') }
  const openMyInvitation = () => {
    if (!activeWedding?.slug) { setApiError('Wedding chưa có slug để mở thiệp.'); return }
    window.open('/' + encodeURIComponent(activeWedding.slug) + '/invitation', '_blank', 'noopener,noreferrer')
  }
  const requestEditorAction = (action: 'invitation' | 'publish') => {
    if (hasUnsavedChanges()) { setPendingEditorAction(action); return }
  ...
  const publish = async () => {
    ...
    try {
      ...
      await weddingApi.publish(activeWedding.id, { surface: 'ONLINE_INVITATION', revision: contentRevision })
      setPublishOpen(false); setPublishSlug(slug); setSurfacePublished(true); setSaveMessage('Thiệp đã được xuất bản thành công.'); await notifications.success('Đã công khai thiệp'); await workspace?.refresh()
  ...
  const unpublish = async () => {
    ...
    setPublishing(true); setApiError('')
    try {
      await weddingApi.unpublish(activeWedding.id, { surface: 'ONLINE_INVITATION', revision: contentRevision ?? undefined })
      setSurfacePublished(false); setSaveMessage('Đã gỡ xuất bản thiệp. Nội dung chỉnh sửa vẫn được giữ nguyên.'); await notifications.success('Đã tạm đóng thiệp'); await loadContent(); await workspace?.refresh()
  ...
  return <section className="invitation-editor" aria-labelledby="invitation-editor-heading">
    ...
    {shareOpen && invitationUrl ? <SharePublicationModal url={invitationUrl} surface="Thiệp online" onClose={() => setShareOpen(false)} /> : null}
    
    <div className="editor-workspace editor-workspace-two-column">
      <aside className="editor-sections editor-section-accordion" aria-label="Cấu trúc và nội dung thiệp" aria-busy={loading}><header><div><strong>Cấu trúc thiệp</strong><small>{loading ? 'Đang tải nội dung đã lưu…' : 'Mở từng phần để chỉnh sửa nội dung'}</small></div>{activeWedding?.status === 'PUBLISHED' ? <div className="editor-structure-publication-status"><span>Thiệp đang được công khai</span>{invitationUrl ? <a href={invitationUrl} target="_blank" rel="noreferrer">{invitationUrl}</a> : null}</div> : null}</header><ol>{order.map((key, index) => <EditorSectionCard key={key} sectionKey={key} definition={sectionDefinitions.find((section) => section.sectionKey === key)} index={index} order={order} expanded={expandedSection === key} shown={enabled.includes(key)} select={() => selectSection(key)} focus={() => { setSelected(key); focusPreviewSection(key) }} move={(step) => { move(key, step); setDirty(true); focusPreviewSection(key) }} toggle={() => { toggle(key); setDirty(true) }}><Fields section={key} definition={sectionDefinitions.find((section) => section.sectionKey === key)} data={data} palette={palette} fieldErrors={fieldErrors} update={update} setPalette={choosePalette} paletteOptions={paletteOptions} showPalette={index === 0} uploadMusic={uploadMusic} openMediaManager={openMediaManager} useMediaManager={Boolean(activeWedding)} musicError={musicError} /></EditorSectionCard>)}</ol></aside>
      <main className={`editor-canvas editor-iframe-canvas ${mobilePreviewOpen ? 'is-mobile-preview-open' : ''}`} aria-label="Bản xem trước thiệp" style={mobilePreviewOpen ? undefined : draggablePreview.style} {...draggablePreview.dragHandlers}><button className="editor-mobile-preview-toggle" type="button" onClick={() => setMobilePreviewOpen((current) => !current)} aria-expanded={mobilePreviewOpen}><span className="editor-preview-drag-handle" data-preview-drag-handle>{mobilePreviewOpen ? <X /> : <ArrowsOut />}</span><span>{mobilePreviewOpen ? 'Thu nhỏ' : 'Xem thiệp'}</span></button><EditorPreviewFrame device={previewDevice} templateKey={templateKey} route={previewPath} frameRef={frameRef} onLoad={sendState} ready={ready} /><button className="editor-mobile-preview-hitbox" type="button" onClick={() => setMobilePreviewOpen(true)} aria-label="Mở rộng bản xem trước thiệp" /></main>
    </div>
    <MobileEditorAdviceModal open={mobileAdviceOpen} onClose={() => setMobileAdviceOpen(false)} />
    {pendingEditorAction ? <div className="editor-publish-backdrop"><section className="editor-publish-dialog" role="dialog" aria-modal="true" aria-labelledby="unsaved-action-title"><FloppyDisk size={28} /><h2 id="unsaved-action-title">Bạn có thay đổi chưa lưu</h2><p>{pendingEditorAction === 'publish' ? 'Hãy lưu nội dung trước khi công khai thiệp.' : 'Hãy lưu nội dung trước khi xem thiệp thực tế của bạn.'}</p><footer className="editor-unsaved-actions"><button className="button button-secondary" type="button" onClick={() => setPendingEditorAction(null)}>Ở lại</button>{pendingEditorAction === 'invitation' ? <button className="button button-secondary" type="button" onClick={() => { setPendingEditorAction(null); openMyInvitation() }}>Tiếp tục không lưu</button> : null}<button className="button button-primary" type="button" disabled={saving} onClick={() => void saveAndContinueEditorAction()}>{saving ? 'Đang lưu…' : 'Lưu và tiếp tục'}</button></footer></section></div> : null}
    {unpublishOpen ? <div className="editor-publish-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setUnpublishOpen(false) }}><section className="editor-publish-dialog" role="dialog" aria-modal="true" aria-labelledby="unpublish-dialog-title"><RocketLaunch size={28} /><h2 id="unpublish-dialog-title">Tạm đóng thiệp?</h2><p>Sau khi tạm đóng, khách mời sẽ không thể truy cập đường dẫn thiệp công khai. Nội dung đã lưu vẫn được giữ nguyên để bạn có thể công khai lại bất cứ lúc nào.</p><footer><button className="button button-secondary" type="button" onClick={() => setUnpublishOpen(false)}>Hủy</button><button className="button button-primary" type="button" disabled={publishing} onClick={() => { setUnpublishOpen(false); void unpublish() }}>{publishing ? 'Đang xử lý…' : 'Tạm đóng thiệp'}</button></footer></section></div> : null}
    ...
    {publishOpen ? <div className="editor-publish-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setPublishOpen(false) }}><section className="editor-publish-dialog" role="dialog" aria-modal="true" aria-labelledby="publish-dialog-title"><RocketLaunch size={28} /><h2 id="publish-dialog-title">Công khai thiệp online</h2><p>Thiệp sẽ dùng đường dẫn wedding do hệ thống cấp để khách mời truy cập.</p><div className="editor-publish-url"><span className="editor-publish-url-label">Đường dẫn thiệp</span><a className="editor-publish-url-value" href={invitationUrl || '#'} target="_blank" rel="noreferrer">{invitationUrl || 'Chưa có slug'}</a></div><footer><button className="button button-secondary" type="button" onClick={() => setPublishOpen(false)}>Hủy</button><button className="button button-primary" type="button" disabled={publishing || !activeWedding?.slug} onClick={() => void publish()}>{publishing ? 'Đang công khai…' : 'Công khai thiệp'}</button></footer></section></div> : null}
  </section>
}
