...
describe('TemplatesApiPage', () => {
  it('loads the online invitation catalog and applies a template', async () => {
    vi.spyOn(weddingApi, 'templates').mockResolvedValue({
      items: [
        {
          key: 'modern-luxe',
          name: 'Élan d’Amour',
          ...
          versions: [
            {
              ...
              config: { sections: ['cover', 'invitation'] },
    ...
    const save = vi.spyOn(weddingApi, 'saveContent').mockResolvedValue({
      content: {
        ...
        surface: 'ONLINE_INVITATION',
        themeConfig: {},
        sectionConfig: { enabled: ['cover', 'invitation'], order: ['cover', 'invitation'] },
        templateVersion: {
          id: '22222222-2222-4222-8222-222222222222',
          key: 'modern-luxe',
          version: '2.3.0',
    ...
    render(
      <NavigationProvider>
        ...
        >
          <TemplatesApiPage kind="invitation" />
        </WeddingContext.Provider>
      </NavigationProvider>,
    )
  ...
  it('keeps the selected deprecated version visible with a warning', async () => {
    ...
    expect(
      await screen.findByText('Giao diện bạn đang dùng đã ngừng phân phối'),
    ).toBeInTheDocument()
