...
import {
  ...
  studioRoutes as baseStudioRoutes,
...
function toTheme(template: WeddingTemplate): Theme | null {
  ...
  return {
    ...
    style:
      template.styles?.[0]?.name ??
      (typeof version.config.style === 'string'
...
function sectionKeyOf(section: TemplateSectionConfig): string {
  ...
  return typeof legacySection.sectionKey === 'string'
    ? legacySection.sectionKey
    : typeof legacySection.key === 'string'
      ? legacySection.key
      : ''
}
...
function sectionDefaults(
  sections: TemplateSectionConfig[],
  previous?: WeddingContent['sectionConfig'],
) {
  ...
  const required = sections
    .filter((item) => typeof item !== 'string' && item.required)
    .map(sectionKeyOf)
    .filter(Boolean)
  const enabled = [
    ...new Set([...previous.enabled.filter((key) => order.includes(key)), ...added, ...required]),
  ]
...
function Artwork({ theme }: { theme: Theme }) {
  return (
    <div
      className={`theme-artwork theme-${theme.key}`}
      role="img"
...
export function TemplatesApiPage({ kind }: { kind: 'invitation' | 'website' }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Không thể tải kho giao diện.')
    }
    if (activeWedding) {
      try {
        setContent((await weddingApi.content(activeWedding.id, surface)).content)
      } catch (cause) {
        await notifications.fire({
          icon: 'error',
          title: 'Không thể tải trạng thái giao diện',
          text: cause instanceof Error ? cause.message : 'Vui lòng thử lại sau.',
  ...
  const unavailableActiveTheme = useMemo<Theme | null>(() => {
    ...
    return {
      ...
      name: localNames[selected.key] ?? selected.key,
      description:
        'Version này đã ngừng phân phối. Bạn vẫn có thể tiếp tục chỉnh sửa thiệp hiện tại.',
  ...
  const visible = useMemo(() => {
    ...
    const matches = themes.filter(
      (theme) =>
        !value ||
        `${theme.name} ${theme.style} ${theme.palette}`.toLocaleLowerCase('vi').includes(value),
    )
    const active =
      themes.find((theme) => theme.versionId === activeVersionId) ?? unavailableActiveTheme
    return active
  ...
  const selectTheme = async (theme: Theme) => {
    ...
    try {
      const saved = await weddingApi.saveContent(activeWedding.id, {
        ...
        sectionConfig: sectionDefaults(theme.sections, content.sectionConfig),
        revision: content.revision,
      })
      setContent(saved.content)
      setNotice(`Đã chọn giao diện ${theme.name}.`)
    } catch (cause) {
      if (
        cause instanceof Error &&
        'code' in cause &&
        cause.code === 'WEDDING_CONTENT_REVISION_CONFLICT'
      ) {
        await notifications.fire({
          icon: 'warning',
          title: 'Dữ liệu vừa thay đổi',
          text: 'Giao diện chưa được áp dụng. Danh sách sẽ được tải lại.',
          confirmButtonText: 'Đã hiểu',
        })
        await load()
      } else {
        await notifications.fire({
          icon: 'error',
          title: 'Không thể dùng giao diện',
          text: cause instanceof Error ? cause.message : 'Vui lòng thử lại sau.',
          confirmButtonText: 'Đã hiểu',
        })
      }
  ...
  const studioRoutes = {
    ...baseStudioRoutes,
    invites: isWebsite ? baseStudioRoutes.siteEditor : baseStudioRoutes.invites,
  }
  ...
  return (
    <section className="templates-page" aria-labelledby="templates-heading">
      <header className="templates-heading">
        <div>
          <p className="breadcrumb">
            ...
            {isWebsite ? 'Website cưới' : 'Thiệp online'} <span>/</span> Kho giao diện
          </p>
          <h1 id="templates-heading">Chọn giao diện {isWebsite ? 'website' : 'thiệp'}</h1>
          <p>
            Kho giao diện được đồng bộ từ hệ thống. Nội dung hiện tại được giữ nguyên khi đổi mẫu.
          </p>
        </div>
        {activeKey ? (
          <div className="current-theme">
            ...
            <span>
              Đang dùng
              <strong>
                {themes.find((item) => item.versionId === activeVersionId)?.name ??
                  unavailableActiveTheme?.name ??
                  activeKey}
              </strong>
            </span>
          </div>
      ...
      {unavailableActiveTheme ? (
        <div className="templates-retired-warning" role="status">
          <WarningCircle size={20} weight="fill" />
          <div>
            <strong>Giao diện bạn đang dùng đã ngừng phân phối</strong>
            <p>
              Bạn vẫn có thể tiếp tục chỉnh sửa thiệp hiện tại. Nếu đổi sang giao diện khác, bạn sẽ
              không thể chọn lại version này.
            </p>
          </div>
        </div>
      ) : null}
      <div className="templates-controls">
        ...
        <label className="template-style-filter">
          <span className="sr-only">Lọc phong cách</span>
          <NativeSelectField value={filter} onChange={(event) => setFilter(event.target.value)}>
      ...
      {loading ? (
        <div className="theme-grid" aria-label="Đang tải kho giao diện">
          {[1, 2, 3].map((item) => (
            <div className="theme-card theme-card-skeleton" key={item}>
              <span />
              <div>
                <i />
                <i />
        ...
        <div className="templates-empty templates-error">
          <WarningCircle size={30} />
          <h2>Chưa tải được kho giao diện</h2>
          <p>{error}</p>
        ...
        <div className="theme-grid">
          {visible.map((theme) => {
            ...
            return (
              <article
                className={`theme-card ${active ? 'is-active' : ''} ${theme.unavailable ? 'is-unavailable' : ''}`}
                key={theme.versionId}
              >
                <div className="theme-preview-wrap">
                  <Artwork theme={theme} />
                  {active ? (
                    <span className="theme-selected" aria-label="Đang dùng" title="Đang dùng">
                      <Check size={18} weight="bold" />
                    </span>
                  ) : null}
                  {theme.unavailable ? (
                    <span className="theme-retired-badge">Ngừng phân phối</span>
                  ) : null}
                </div>
                ...
                <footer>
                  {theme.previewPath ? (
                    <AppLink className="button button-secondary" to={theme.previewPath}>
                  ...
                  {active ? (
                    ...
                    <button
                      ...
                      type="button"
                      disabled={theme.unavailable || saving !== null || !activeWedding}
        ...
        <div className="templates-empty">
          ...
          <h2>{themes.length ? 'Không tìm thấy giao diện' : 'Chưa có giao diện khả dụng'}</h2>
          <p>
            {themes.length
              ? 'Thử đổi từ khóa hoặc bộ lọc.'
              : 'Template sẽ xuất hiện sau khi được phát hành từ hệ thống.'}
          </p>
          {themes.length ? (
            <button
      ...
      {notice ? (
        <div className="templates-feedback" role="status">
          <Check className="templates-success-check" size={34} weight="bold" />
          <span>{notice}</span>
