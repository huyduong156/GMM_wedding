...
const toLabel = (date: string | null) =>
  date ? new Date(date).toLocaleDateString('vi-VN') : 'Chưa đặt hạn'
...
export function TodosPage() {
  ...
  const [query, setQuery] = useState(''),
    [filter, setFilter] = useState<'all' | TaskStatus>('all')
  const [sort, setSort] = useState<'due' | 'priority'>('due')
  const [open, setOpen] = useState(false),
    [editing, setEditing] = useState<WeddingTask | null>(null),
    [templatesOpen, setTemplatesOpen] = useState(false),
    [saving, setSaving] = useState(false),
    [applyingTemplateKey, setApplyingTemplateKey] = useState<string | null>(null)
  const [title, setTitle] = useState(''),
    [dueAt, setDueAt] = useState(''),
    [priority, setPriority] = useState<TaskPriority>('MEDIUM'),
    [parentTaskId, setParentTaskId] = useState<string | null>(null)
      ...
      ...tasks
        .filter(
          (task: WeddingTask) =>
            ...
            (!deferredQuery ||
              `${task.title} ${task.description ?? ''} ${task.event?.name ?? ''}`
                .toLocaleLowerCase('vi')
                .includes(deferredQuery)),
        )
        .sort((a: WeddingTask, b: WeddingTask) =>
          sort === 'priority'
            ? priorityRank(b.priority) - priorityRank(a.priority)
            : (a.dueAt ?? '9999').localeCompare(b.dueAt ?? '9999'),
        ),
    ],
    [tasks, filter, deferredQuery, sort],
  )
  const orderedTasks = useMemo(() => {
    const roots = visibleTasks.filter((task) => !task.parentTaskId)
    return roots.flatMap((root) => [
      root,
      ...visibleTasks.filter((task) => task.parentTaskId === root.id),
    ])
  }, [visibleTasks])
    ...
    try {
      ...
      await showError(
        cause instanceof WeddingApiError && cause.status === 409
          ? 'Công việc vừa thay đổi ở nơi khác. Danh sách đã được tải lại.'
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      setTasks((items) =>
        editing
          ? items.map((item) => (item.id === result.task.id ? result.task : item))
          : [result.task, ...items],
      )
      setOpen(false)
  ...
  async function applyTemplate(template: TodoChecklistPreset) {
    ...
    const confirmation = await notifications.fire({
      ...
      text:
        template.tasks.length + ' công việc từ “' + template.name + '” sẽ được thêm vào Todolist.',
      showCancelButton: true,
      confirmButtonText: 'Thêm checklist',
      cancelButtonText: 'Hủy',
    })
    if (!confirmation.isConfirmed) return
  ...
  return (
    <div className="todos-page">
        ...
        <div className="todos-heading-actions">
          <div className="todo-template-trigger">
            <button
              ...
              aria-controls="todo-template-popover"
            ...
            {templatesOpen ? (
              <div
                className="todo-template-popover"
                id="todo-template-popover"
                role="region"
                aria-label="Checklist gợi ý"
              >
                <header>
                  <div>
                    <strong>Checklist gợi ý</strong>
                    <span>Chọn một mẫu để thêm nhanh</span>
                  </div>
                  <button
                    type="button"
                    aria-label="Đóng checklist gợi ý"
                ...
                {templates.length ? (
                  templates.slice(0, 3).map((template) => (
                    <button
                      className="todo-template-option"
                      type="button"
                      key={template.key}
                      disabled={applyingTemplateKey !== null}
                      aria-busy={applyingTemplateKey === template.key}
                      onClick={() => setPreviewPreset(template)}
                    >
                      <span>
                        <strong>{template.name}</strong>
                        <small>{template.description}</small>
                      </span>
                      <b>{template.tasks.length}</b>
                      <Plus size={17} />
                    </button>
                  ))
                ) : (
                  <p className="todo-template-empty">Chưa có checklist gợi ý.</p>
                )}
              </div>
            ) : null}
          </div>
      ...
      <section className="todos-overview" aria-label="Tiến độ chuẩn bị">
        <div className="todos-progress-icon">
        ...
        <div
          className="todos-progress-track"
          role="progressbar"
          aria-label={`${progress}% công việc đã hoàn thành`}
        ...
        <section className="panel todos-workspace">
          <div className="todos-toolbar">
            ...
            >
              ...
              {sort === 'due' ? 'Hạn gần nhất' : 'Ưu tiên cao'}
              <CaretDown size={13} />
            </button>
          </div>
          <div className="todos-filters" role="tablist" aria-label="Lọc trạng thái công việc">
          ...
          <div className="todo-list">
            {loading ? (
              <div className="workspace-skeleton" role="status" aria-label="Đang tải công việc">
                <i />
                <i />
                <i />
              </div>
              ...
              orderedTasks.map((task) => (
                <article
                  className={`todo-row ${task.status === 'DONE' ? 'is-done' : ''}`}
                ...
                >
                  <button
                    ...
                    aria-label={
                      task.status === 'DONE'
                        ? `Đánh dấu ${task.title} là chưa hoàn thành`
                        : `Hoàn thành ${task.title}`
                    }
                    aria-pressed={task.status === 'DONE'}
                    onClick={() =>
                      void updateStatus(task, task.status === 'DONE' ? 'TODO' : 'DONE')
                    }
                  >
              ...
              <div className="todos-empty">
                <CheckCircle size={29} />
                <h2>{weddingId ? 'Không có công việc phù hợp' : 'Chưa có wedding đang chọn'}</h2>
                <p>Thử đổi bộ lọc hoặc thêm công việc mới.</p>
              </div>
            )}
          </div>
          <footer className="todos-footer">
            <span>
              Hiển thị {visibleTasks.length} trên {tasks.length} công việc
            </span>
            <span>Đã đồng bộ API</span>
          </footer>
        </section>
      </div>
      {previewPreset ? (
        <div
          ...
          role="presentation"
          onMouseDown={() => setPreviewPreset(null)}
        >
          <section
            className="todo-dialog todo-checklist-preview"
            role="dialog"
            aria-modal="true"
            aria-labelledby="todo-checklist-preview-title"
            onMouseDown={(event) => event.stopPropagation()}
          >
            <header>
              <div>
                <h2 id="todo-checklist-preview-title">{previewPreset.name}</h2>
                <p>{previewPreset.description}</p>
              </div>
              <button
                type="button"
                aria-label="Đóng xem trước checklist"
                onClick={() => setPreviewPreset(null)}
              >
                <X size={18} />
              </button>
            </header>
            <div className="todo-checklist-preview-list">
              {previewPreset.tasks.map((item) => (
                <div className="todo-checklist-preview-item" key={item.title}>
                  <CheckCircle size={18} />
            ...
            <footer>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  void applyTemplate(previewPreset)
                }}
              >
                {applyingTemplateKey ? 'Đang thêm…' : 'Áp dụng checklist'}
              </button>
            </footer>
          </section>
        </div>
      ...
      {open ? (
        <div className="todo-dialog-backdrop" role="presentation">
          ...
          >
            <header>
              <h2 id="todo-dialog-title">{editing ? 'Chỉnh sửa công việc' : 'Thêm công việc'}</h2>
            ...
            <div className="todo-dialog-grid todo-dialog-meta">
              <label>
                Hạn hoàn thành
                <NativeDateField
                  name="dueAt"
              ...
              <label>
                ...
                <NativeSelectField
                  name="parentTaskId"
                  value={parentTaskId ?? ''}
                  onChange={(event) => setParentTaskId(event.target.value || null)}
                >
                  <option value="">Không có — công việc chính</option>
                  {tasks
                    .filter((task) => !task.parentTaskId && task.id !== editing?.id)
                    .map((task) => (
                ...
                <small>Chỉ hỗ trợ task con tối đa 1 cấp.</small>
              </label>
            </div>
            <footer>
              <button
