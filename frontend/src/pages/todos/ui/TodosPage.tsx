...
const showError = (text: string) => notifications.fire({ icon: 'error', title: 'Không thể thực hiện', text, confirmButtonText: 'Đã hiểu' })
const showSuccess = (title: string) => notifications.fire({ icon: 'success', title, timer: 1400, timerProgressBar: true, showConfirmButton: false })
...
export function TodosPage() {
  ...
  async function remove(task: WeddingTask) {
    if (!weddingId) return
    const confirmation = await notifications.fire({ icon: 'warning', title: 'Xóa công việc?', text: 'Công việc ' + task.title + ' sẽ bị xóa.', showCancelButton: true, confirmButtonText: 'Xóa công việc', cancelButtonText: 'Hủy', confirmButtonColor: '#a43d34' })
  ...
  async function applyTemplate(template: TodoChecklistPreset) {
    ...
    const confirmation = await notifications.fire({ icon: 'warning', title: 'Thêm checklist?', text: template.tasks.length + ' công việc từ “' + template.name + '” sẽ được thêm vào Todolist.', showCancelButton: true, confirmButtonText: 'Thêm checklist', cancelButtonText: 'Hủy' })
    if (!confirmation.isConfirmed) return
