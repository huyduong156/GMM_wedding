...
const toTab = (status: WishStatus): WishTab =>
  status === 'APPROVED' ? 'approved' : status === 'PENDING' ? 'pending' : 'hidden'
...
function WishesContent({ activeWedding }: { activeWedding: { id: string; name: string } | null }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Không thể tải lời chúc.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const moderate = async (
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Không thể cập nhật lời chúc.')
    }
  }
      ...
      <header className="wishes-heading">
        <div>
          <p className="breadcrumb">
            {activeWedding?.name ?? 'Đám cưới'} <span>/</span> Lời chúc
          </p>
        ...
        <div className="wishes-highlight" aria-label="Tổng số lời chúc đã nhận">
          ...
          <span>
            <strong>{counts.total}</strong> lời chúc
          </span>
        </div>
      </header>
      <div className="panel wishes-workspace">
        ...
        {loading ? (
          <div className="wishes-empty">
            <Sparkle size={28} />
            <h2>Đang tải lời chúc…</h2>
          </div>
        ) : error ? (
          <div className="wishes-empty">
            <X size={28} />
            <h2>Không thể tải lời chúc</h2>
            <p>{error}</p>
          ...
          <div className="wishes-list" role="tabpanel">
            {visibleWishes.length ? (
              visibleWishes.map((wish) => {
                const author = wish.guestName || wish.authorName || 'Khách mời'
                const isApproved = wish.status === 'APPROVED'
                const isHidden = toTab(wish.status) === 'hidden'
                return (
                  <article
                    className={`wish-card ${wish.isPinned ? 'is-pinned' : ''}`}
                    key={wish.id}
                  >
                    ...
                    <div className="wish-content">
                      <header>
                        <div>
                          ...
                          <span>
                            {wish.authorName !== author ? wish.authorName : 'Khách mời'} ·{' '}
                            {toDate(wish.submittedAt)}
                          </span>
                        </div>
                        {wish.isPinned ? (
                      ...
                      <div className="wish-actions">
                        ...
                        {isApproved ? (
                          <button
                            ...
                            aria-pressed={wish.isPinned}
                            onClick={() =>
                              void moderate(
                                wish,
                                { isPinned: !wish.isPinned },
                                wish.isPinned ? 'Đã bỏ ghim lời chúc' : 'Đã ghim lời chúc',
                              )
                            }
                          >
                            <PushPin size={16} /> {wish.isPinned ? 'Bỏ ghim' : 'Ghim'}
                          </button>
                        ) : null}
                        {!isHidden ? (
                          <button
                            ...
                            onClick={() =>
                              void moderate(
                                wish,
                                { status: 'HIDDEN', isPinned: false },
                                `Đã ẩn lời chúc của ${author}`,
                              )
                            }
                          >
                            <EyeSlash size={16} /> Ẩn
                          ...
                          <button
                            ...
                            onClick={() =>
                              void moderate(
                                wish,
                                { status: 'PENDING' },
                                `Đã chuyển lời chúc của ${author} về chờ duyệt`,
                              )
                            }
                          >
