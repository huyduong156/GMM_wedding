...
import { Bank, CaretDown, Coins, CurrencyCircleDollar, DotsThree, Eye, EyeSlash, Gift, LockKey, MagnifyingGlass, Plus, ShieldCheck, X } from '@phosphor-icons/react'
...
export function GiftLedgerPage() {
  ...
  const removeEntry = useCallback(async (entry: GiftEntry) => {
    if (!weddingId) return
    const confirmation = await Swal.fire({ icon: 'warning', title: 'Xóa khoản mừng?', text: 'Khoản của “' + entry.guestName + '” sẽ bị xóa khỏi sổ.', showCancelButton: true, confirmButtonText: 'Xóa khoản mừng', cancelButtonText: 'Hủy', confirmButtonColor: '#a43d34' })
  ...
  async function unlinkGuest(entry: GiftEntry) {
    ...
    const confirmation = await Swal.fire({ icon: 'warning', title: 'Gỡ liên kết khách mời?', text: 'Khoản mừng vẫn được giữ lại dưới dạng vô danh.', showCancelButton: true, confirmButtonText: 'Gỡ liên kết', cancelButtonText: 'Hủy', confirmButtonColor: '#a45d08' })
    if (!confirmation.isConfirmed) return
...
function GiftTableRow({ entry, revealed }: { entry: GiftEntry; revealed: boolean }) { return <tr><td><Identity entry={entry} /></td><td><span className="gift-value">{entryValue(entry, revealed)}</span></td><td><span className="gift-method">{entry.method === 'bank' ? <Bank size={14} /> : entry.kind === 'gold' ? <Coins size={14} /> : entry.method === 'physical' ? <Gift size={14} /> : <CurrencyCircleDollar size={14} />}{entry.kind === 'gold' ? 'Vàng' : methodLabels[entry.method]}</span></td><td>{entry.receivedAt}</td><td><span className={'gift-reciprocity ' + entry.reciprocity}>{reciprocityLabels[entry.reciprocity]}</span></td><td><span className="gift-note">{entry.note ?? '—'}</span></td><td className="gift-table-actions"><GiftActions entry={entry} /></td></tr> }
...
function GiftCard({ entry, revealed }: { entry: GiftEntry; revealed: boolean }) { return <article><header><Identity entry={entry} /><div className="gift-card-actions"><span className="gift-value">{entryValue(entry, revealed)}</span><GiftActions entry={entry} /></div></header><dl><div><dt>Hình thức</dt><dd>{entry.kind === 'gold' ? 'Vàng' : methodLabels[entry.method]}</dd></div><div><dt>Ngày nhận</dt><dd>{entry.receivedAt}</dd></div><div><dt>Trạng thái</dt><dd><span className={'gift-reciprocity ' + entry.reciprocity}>{reciprocityLabels[entry.reciprocity]}</span></dd></div><div><dt>Ghi chú</dt><dd>{entry.note ?? '—'}</dd></div></dl></article> }
