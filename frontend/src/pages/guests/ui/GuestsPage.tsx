...
function StatusBadge({ status }: { status: RsvpStatus }) {
  ...
  return (
    <span className={`guest-status ${status}`}>
    ...
    <section className="guests-page">
      ...
      <div className="guest-summary" aria-label="Tổng quan khách mời">
        <div>
          ...
          <strong>128</strong>
          <small>186 người dự kiến</small>
        </div>
        <div>
          <span>
            <CheckCircle size={17} /> Có tham dự
          </span>
          <strong>74</strong>
          <small>58% danh sách</small>
        </div>
        <div>
          <span>
            <Clock size={17} /> Chưa xác nhận
          </span>
          <strong>46</strong>
          <small>Cần xác nhận lại</small>
        </div>
        <div>
          <span>
      ...
      <div className="guest-directory panel">
        <div className="guest-toolbar">
          <label className="guest-search">
            ...
            <input
              ...
              placeholder="Tìm nhanh theo tên khách…"
            />
          </label>
          ...
          <label className="guest-group-filter guest-tag-filter">
            ...
            <NativeSelectField value={tag} onChange={(event) => setTag(event.target.value)}>
              ...
              <optgroup label="Công ty">
                {guestTags
                  .filter((item) => item.startsWith('Công ty'))
                  .map((item) => (
                    <option key={item}>{item}</option>
                  ))}
              </optgroup>
              <optgroup label="Họ hàng">
                {guestTags
                  .filter((item) => ['Họ nội', 'Họ ngoại', 'Nhà trai', 'Nhà gái'].includes(item))
                  .map((item) => (
                    <option key={item}>{item}</option>
                  ))}
              </optgroup>
              <optgroup label="Bạn bè">
                {guestTags
                  .filter(
                    (item) =>
                      !item.startsWith('Công ty') &&
                      !['Họ nội', 'Họ ngoại', 'Nhà trai', 'Nhà gái'].includes(item),
                  )
                  .map((item) => (
                    <option key={item}>{item}</option>
                  ))}
          ...
          <button className="button button-secondary guest-export" type="button">
            <DownloadSimple size={16} /> Xuất file
          </button>
        </div>
        ...
        {filteredGuests.length ? (
                  ...
                  {filteredGuests.map((guest) => (
                    <tr key={guest.id} className={selected.includes(guest.id) ? 'is-selected' : ''}>
                      <td>
                        <GuestCheckbox
                          guest={guest}
                          selected={selected.includes(guest.id)}
                      ...
                      <td>
                        <div className="guest-identity">
                          ...
                          <div>
                            <button type="button">{guest.name}</button>
                            <small>{guest.tags.join(' · ')}</small>
                          </div>
                        </div>
                      </td>
                      <td>
                        <span className="guest-group-tag">{guest.group}</span>
                      </td>
                      <td>{guest.partySize}</td>
                      <td>
                        <span
                          className={
                            guest.invitation === 'Đã gửi' ? 'invite-state sent' : 'invite-state'
                          }
                        >
                          {guest.invitation}
                        </span>
                      ...
                      <td>
                        <button
                          className="row-menu"
                          type="button"
                          aria-label={`Mở thao tác cho ${guest.name}`}
                        >
              ...
              {filteredGuests.map((guest) => (
                ...
                >
                  ...
                  <div className="guest-card-status">
                    <StatusBadge status={guest.attendance} />
                    <span className="guest-group-tag">{guest.group}</span>
                    {guest.tags.slice(0, 1).map((item) => (
                      <span className="guest-tag" key={item}>
                        {item}
                      </span>
                    ))}
                  </div>
          ...
          <div className="guest-empty">
            <Users size={28} />
            <h2>Không tìm thấy khách mời</h2>
            <p>Thử đổi từ khóa hoặc bỏ bớt bộ lọc đang áp dụng.</p>
            <button
        ...
        <footer className="guest-pagination">
          <span>
            Đang hiển thị <strong>1–50</strong> trong 128 khách
          </span>
          <label className="guest-page-size">
            Số dòng{' '}
            <NativeSelectField defaultValue="50">
              <option>50</option>
              <option>100</option>
              <option>200</option>
            </NativeSelectField>
