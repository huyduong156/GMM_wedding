...
const content: Record<
  StatusKind,
  { code: string; title: string; description: string; Icon: StatusIcon }
> = {
  unauthorized: {
    code: '401',
    title: 'Bạn cần đăng nhập để tiếp tục',
    description:
      'Phiên làm việc chưa sẵn sàng hoặc đã hết hạn. Hãy đăng nhập lại để truy cập nội dung này.',
    Icon: LockKey,
  },
  forbidden: {
    code: '403',
    title: 'Bạn không có quyền truy cập',
    description: 'Tài khoản hiện tại không được phép xem hoặc thực hiện thao tác này.',
    Icon: LockKey,
  },
  'not-found': {
    code: '404',
    title: 'Không tìm thấy trang',
    description: 'Đường dẫn này không tồn tại hoặc nội dung đã được chuyển sang một nơi khác.',
    Icon: WarningCircle,
  },
  'server-error': {
    code: '500',
    title: 'Đã có lỗi xảy ra',
    description: 'Hệ thống gặp sự cố khi tải trang. Bạn có thể thử lại hoặc quay về studio.',
    Icon: Warning,
  },
}
  ...
  return (
    <main className="status-page" aria-labelledby="status-page-title">
      <div className="status-page-card">
        ...
        <span className="status-page-code">{item.code}</span>
        <h1 id="status-page-title">{item.title}</h1>
        ...
        <div className="status-page-actions">
          ...
          <AppLink
            className={onRetry ? 'button button-secondary' : 'button button-primary'}
            to={home}
          >
            <House size={16} />
            {homeLabel}
          ...
          {kind !== 'unauthorized' ? (
            <AppLink className="status-page-back" to={marketingRoutes.home}>
