...
const wedding = {
  ...
  timezone: 'Asia/Ho_Chi_Minh',
  locale: 'vi-VN',
  primaryDate: '2099-12-12T05:00:00Z',
  revision: 1,
  publishedAt: null,
  archivedAt: null,
...
describe('connected Wedding workspace', () => {
  it('shows onboarding and creates the first wedding through the API', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation((input, init) => {
      ...
      if (url.endsWith(`/weddings/${wedding.id}/dashboard`))
        return json({
          dashboard: {
            wedding,
            publication: {
        ...
        return json({
          dashboard: {
            ...
            metrics: {
              guests: 12,
              invitations: 10,
              activeInvitations: 10,
              responses: 7,
              attending: 6,
              declined: 1,
              maybe: 0,
              pendingResponses: 3,
              attendingPartySize: 8,
              companions: 2,
              pendingWishes: 0,
              approvedWishes: 0,
            },
            nextEvent: null,
            responseTrend: Array.from({ length: 30 }, (_, index) => ({
