...
export function PeonyVerandaPreviewPage() {
  ...
  const [sectionConfig, setSectionConfig] = useState<PeonyVerandaSectionConfig | undefined>(stored?.sectionConfig)
  ...
  return <>{!editorMode ? <aside className='pv-preview-toolbar' aria-label='Tùy chọn xem trước'><div><strong>Peony Veranda</strong><span>Peony botanical invitation · v0.1</span></div><a href='/studio/invites/themes'>Đóng xem trước</a></aside> : null}<PeonyVerandaInvitation editorMode={editorMode} data={data} sectionConfig={sectionConfig} /></>
}

function readPreviewState(): { data?: PeonyVerandaData; sectionConfig?: PeonyVerandaSectionConfig } | null {
  try { return JSON.parse(sessionStorage.getItem('gmm-invitation-preview:peony-veranda') ?? 'null') as { data?: PeonyVerandaData; sectionConfig?: PeonyVerandaSectionConfig } | null } catch { return null }
}
