...
const rsvpController = { isPersonalized: false, submit: vi.fn(), submitting: false, submitted: false, error: '' }
const wishesController = { items: [], submit: vi.fn(), submitting: false, submitted: false, error: '' }

vi.mock('../../../features/auth/model/auth-context', () => ({ useOptionalAuth: () => undefined }))
vi.mock('../../../shared/lib/navigation/usePublicRsvp', () => ({ usePublicRsvp: () => rsvpController }))
vi.mock('../../../shared/lib/navigation/usePublicWishes', () => ({ usePublicWishes: () => wishesController }))
...
const snapshot = {
  id: 'snapshot-1', weddingId: 'wedding-1', surface: 'ONLINE_INVITATION' as const, slug: 'mai-duc', version: 1,
  publishedAt: '2026-01-01T00:00:00.000Z', payload: { template: { key: 'modern-luxe' }, content: {}, theme: {} },
...
describe('PublicInvitationPage guest identity flow', () => {
  it('loads the common invitation without calling the guest endpoint', async () => {
    ...
    invitation.mockRestore(); guest.mockRestore()
  })

  it('loads the guest identity once and passes guestName through interactions', async () => {
    const invitation = vi.spyOn(weddingApi, 'publicInvitation').mockResolvedValue({ snapshot })
    const guest = vi.spyOn(weddingApi, 'publicInvitationGuest').mockResolvedValue({ invitation: { weddingSlug: 'mai-duc', invitationSlug: 'anh-ba-hung', guestName: 'Anh Ba Hưng', maxPartySize: 2, expiresAt: null } })
  ...
  it('keeps the common snapshot renderable while a guest lookup fails temporarily', async () => {
    const invitation = vi.spyOn(weddingApi, 'publicInvitation').mockResolvedValue({ snapshot })
    const guest = vi.spyOn(weddingApi, 'publicInvitationGuest').mockRejectedValue(new Error('Network error'))
  ...
  it('shows not found when the guest slug is invalid', async () => {
    const invitation = vi.spyOn(weddingApi, 'publicInvitation').mockResolvedValue({ snapshot })
    const guest = vi.spyOn(weddingApi, 'publicInvitationGuest').mockRejectedValue(new WeddingApiError(404, 'RESOURCE_NOT_FOUND', 'Guest link not found'))
