...
function resetError(reason: unknown) {
  ...
  if (reason.code === 'INVALID_PASSWORD_RESET_TOKEN')
    ...
    <AuthRecoveryLayout titleId="reset-password-title">
      <form className="login-card auth-recovery-card" onSubmit={submit}>
        ...
        {complete ? (
          <div className="auth-form-success" role="status">
            ...
            <div>
              ...
              <span>Bạn có thể đăng nhập lại bằng mật khẩu mới.</span>
            </div>
          </div>
        ) : token ? (
          <>
        ...
        <AppLink className="auth-back-link" to={marketingRoutes.login}>
          <ArrowLeft size={16} /> {complete ? 'Đăng nhập ngay' : 'Quay lại đăng nhập'}
        </AppLink>
      </form>
    </AuthRecoveryLayout>
  )
