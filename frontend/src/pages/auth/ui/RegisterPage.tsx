      ...
      <form className="login-card auth-recovery-card" onSubmit={submit}>
        ...
        {sent ? (
          <>
            ...
            <ResendVerificationControl initialEmail={email} />
          </>
        ) : (
          <>
            <label htmlFor="register-name">
              Tên hiển thị <span className="auth-optional">(không bắt buộc)</span>
            </label>
            <input
              id="register-name"
              type="text"
        ...
        <AppLink className="auth-back-link" to={marketingRoutes.login}>
          <ArrowLeft size={16} /> {sent ? 'Về trang đăng nhập' : 'Đã có tài khoản? Đăng nhập'}
        </AppLink>
      </form>
    </AuthRecoveryLayout>
  )
