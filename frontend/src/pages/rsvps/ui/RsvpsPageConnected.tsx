...
const statusCopy: Record<RsvpAttendance, string> = {
...
const eventLabel = (response: Rsvp) =>
  response.eventSelections
...
function RsvpsContent({ activeWedding }: { activeWedding: { id: string; name: string } | null }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Không thể tải phản hồi tham dự.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const filterCounts = {
    all: summary.total,
    ATTENDING: summary.attending.length,
    ...
    <section className="rsvp-page" aria-labelledby="rsvp-heading">
      <header className="rsvp-heading">
        <div>
          <p className="breadcrumb">
            {activeWedding?.name ?? 'Đám cưới'} <span>/</span> Xác nhận tham dự
          </p>
      ...
      <div className="rsvp-summary" aria-label="Tổng quan xác nhận tham dự">
        <article>
          ...
          <small>Đã gửi xác nhận</small>
        </article>
        <article>
          <span>
            <CheckCircle size={17} /> Sẽ tham dự
          </span>
          <strong>{summary.attending.length}</strong>
          <small>
            {summary.attending.reduce((sum, item) => sum + item.partySize, 0)} người dự kiến
          </small>
        </article>
        <article>
          <span>
          ...
          <strong>{summary.maybe.length}</strong>
        ...
        <article>
          ...
          <strong>{summary.declined.length}</strong>
      ...
      <div className="panel rsvp-directory">
        <div className="rsvp-toolbar">
          <label className="rsvp-search">
            <span className="sr-only">Tìm theo tên khách</span>
        ...
        {loading ? (
          <div className="guest-empty">
            <UsersThree size={28} />
            <h2>Đang tải phản hồi…</h2>
          </div>
        ) : error ? (
          <div className="guest-empty">
            <XCircle size={28} />
            <h2>Không thể tải phản hồi</h2>
            <p>{error}</p>
                  ...
                  {responses.map((response) => (
                    <tr key={response.id}>
                      <td>
                        ...
                        <small>{response.invitationLabel ?? 'Thiệp cưới'}</small>
                      </td>
                      <td className="rsvp-status-cell">
                        <span className={`guest-status ${response.attendance.toLowerCase()}`}>
                          {statusCopy[response.attendance]}
                        </span>
                        <small>{eventLabel(response)}</small>
                      </td>
                      <td className="rsvp-party-size">{response.partySize}</td>
                      <td className="rsvp-note">
                        {response.message || response.specialRequest || (
                          <span className="muted-value">Không có ghi chú</span>
                        )}
                        <small>{dateTime(response.submittedAt)}</small>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="rsvp-mobile-list">
              {responses.map((response) => (
                <article className="rsvp-card" key={response.id}>
                  ...
                  {(response.message || response.specialRequest) && (
                    <p>{response.message || response.specialRequest}</p>
                  )}
                  <footer>
                    <span>{response.invitationLabel ?? 'Thiệp cưới'}</span>
                    <time>{dateTime(response.submittedAt)}</time>
                  </footer>
                </article>
              ))}
            </div>
            <footer className="rsvp-footer">
              <span>Hiển thị {responses.length} phản hồi</span>
            </footer>
          </>
        ) : (
          <div className="guest-empty">
            ...
            <p>Các xác nhận tham dự sẽ xuất hiện tại đây.</p>
          </div>
        )}
      </div>
    </section>
