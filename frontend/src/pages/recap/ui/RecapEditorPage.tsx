...
import { ArrowDown, ArrowLeft, ArrowUp, Check, CheckCircle, Copy, Desktop, DeviceMobile, Eye, FacebookLogo, FloppyDisk, ImageSquare, Link, Minus, MusicNotes, PaperPlaneTilt, Plus, Quotes, ShareNetwork, X } from '@phosphor-icons/react'
...
function RecapTextField({ label, path, value, update, multiline = false, maxLength, type = 'text', disabled = false }: { label: string; path: string; value: string; update: (path: string, value: unknown) => void; multiline?: boolean; maxLength?: number; type?: 'text' | 'url'; disabled?: boolean }) {
  const limit = maxLength ?? fieldLimits[path.split('.').at(-1) ?? '']
  return <label className="recap-field"><span>{label}{limit ? <small>{value.length}/{limit}</small> : null}</span>{multiline ? <textarea disabled={disabled} value={value} maxLength={limit} rows={4} onChange={(event) => update(path, event.target.value)} /> : <input disabled={disabled} type={type} value={value} maxLength={limit} onChange={(event) => update(path, event.target.value)} />}</label>
}
...
export function RecapEditorPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRecap(result)
      if (result.status === 'PUBLISHED' && result.slug) { setPublished(true); setShareUrl(window.location.origin + '/public/recaps/' + result.slug) }
  ...
  const openDraftPreview = () => {
    const templateKey = recap?.templateVersion.key ?? 'red-spider-lily-recap'
    const previewRoute = typeof recap?.templateVersion.config.previewPath === 'string' ? recap.templateVersion.config.previewPath : publicTemplateRoutes.redSpiderLilyRecapPreview
    sessionStorage.setItem(`gmm-recap-preview:${templateKey}`, JSON.stringify({ data: content, sectionConfig: { enabled, order } }))
    window.open(`${previewRoute}?draft=1&template=${encodeURIComponent(templateKey)}`, '_blank', 'noopener,noreferrer')
  }

  const shareRecap = async () => {
    if (shareUrl) { setShareOpen(true); return }
    if (!wedding || !recap || saving) return
    const didSave = saved || await save()
    if (!didSave) return
    try {
      const latest = (await weddingApi.recap(wedding.id)).recap
      if (!latest) return
      const slug = latest.slug ?? recapSlug(wedding.name) + '-recap'
      const { snapshot } = await weddingApi.publishRecap(wedding.id, { slug, revision: latest.revision })
      const publicUrl = window.location.origin + '/public/recaps/' + snapshot.slug
      setPublished(true)
    ...
    } catch (cause) { const message = cause instanceof Error ? cause.message : 'Không thể xuất bản recap.'; setError(message); notifications.fire({ toast: true, position: 'top-end', icon: 'error', title: message, showConfirmButton: false, timer: 3200, timerProgressBar: true }) }
  }
  ...
  return <RecapMediaContext.Provider value={{ assets: mediaAssets, selectedIds: selectedMediaIds, openManager: (target, role = 'hero', multiple = false) => { setMediaError(''); setMediaTarget(target ? { path: target, role, multiple } : null); setMediaSelectionMode(multiple ? 'multiple' : 'single'); setMediaManagerOpen(true) } }}><section className="recap-editor-page" aria-labelledby="recap-editor-heading">
    ...
    {shareOpen && shareUrl ? <ShareRecapModal url={shareUrl} copied={copied} onClose={() => setShareOpen(false)} onCopy={async () => { await navigator.clipboard?.writeText(shareUrl); setCopied(true) }} onNativeShare={async () => { if (navigator.share) await navigator.share({ title: 'Wedding recap', url: shareUrl }) }} /> : null}
...
function ShareRecapModal({ url, copied, onClose, onCopy, onNativeShare }: { url: string; copied: boolean; onClose: () => void; onCopy: () => Promise<void>; onNativeShare: () => Promise<void> }) {
  ...
  return <div className='recap-share-backdrop' role='presentation' onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section className='recap-share-dialog' role='dialog' aria-modal='true' aria-labelledby='recap-share-title'>
      <button type='button' className='recap-share-close' onClick={onClose} aria-label='Đóng'><X size={18} /></button>
      <span className='recap-share-kicker'>RECAP ĐÃ SẴN SÀNG</span><h2 id='recap-share-title'>Chia sẻ câu chuyện của hai bạn</h2><p>Gửi đường dẫn recap đến gia đình và bạn bè.</p>
      <div className='recap-share-link'><Link size={17} /><span>{url}</span><button type='button' onClick={() => void onCopy()} aria-label='Sao chép đường dẫn'><Copy size={17} /></button></div>
      <div className='recap-share-actions'><a className='button button-secondary' href={facebookUrl} target='_blank' rel='noreferrer'><FacebookLogo size={17} /> Facebook</a>{typeof navigator !== 'undefined' && 'share' in navigator ? <button className='button button-secondary' type='button' onClick={() => void onNativeShare()}><ShareNetwork size={17} /> Chia sẻ</button> : null}<button className='button button-primary' type='button' onClick={() => void onCopy()}><Copy size={17} /> {copied ? 'Đã sao chép' : 'Sao chép link'}</button></div>
    </section>
  </div>
}
