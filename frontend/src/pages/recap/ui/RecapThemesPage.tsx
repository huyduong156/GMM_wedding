...
const localMeta: Record<string, { name: string; style: string; palette: string }> = {
  'red-spider-lily-recap': {
    name: 'Dấu Son Bỉ Ngạn',
    style: 'Botanical editorial',
    palette: 'Vermilion paper · Ink ivory',
  },
}
...
function toTheme(template: WeddingTemplate): RecapTheme | null {
  ...
  return {
    ...
    description:
      ...
      'Một cách kể lại ngày vui bằng nhịp ảnh, lời kể và những khoảng lặng vừa đủ.',
...
function Artwork({ theme, active }: { theme: RecapTheme; active: boolean }) {
  return (
    <div
      className={`recap-library-art theme-${theme.key}`}
      role="img"
    ...
    >
      <div className="recap-library-art-copy">
        ...
        <small>Đà Lạt · 14.12.2026</small>
      </div>
      {active ? (
        <span className="recap-theme-selected" aria-label="Đang dùng" title="Đang dùng">
...
export function RecapThemesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      try {
        setRecap((await weddingApi.recap(wedding.id)).recap)
      } catch (cause) {
  ...
  const visible = useMemo(() => {
    ...
    return themes.filter(
      (theme) =>
        (!styleFilter || theme.styles.some((style) => style.key === styleFilter)) &&
        (!value ||
  ...
  const applyTheme = async (theme: RecapTheme) => {
    ...
    try {
      const saved = await weddingApi.saveRecap(wedding.id, {
        ...
        sectionConfig:
          recap?.templateVersion.key === theme.key
            ? recap.sectionConfig
            : defaultSectionConfig(theme),
        mediaItems:
          recap?.mediaItems.map((item) => ({
            mediaAssetId: item.mediaAssetId,
            caption: item.caption,
            sortOrder: item.sortOrder,
          })) ?? [],
        wishSelections:
          recap?.wishSelections.map((item) => ({
            wishId: item.wishId,
            sortOrder: item.sortOrder,
          })) ?? [],
      ...
      await toast(
        cause instanceof Error ? cause.message : 'Không thể áp dụng giao diện recap.',
        'error',
      )
    ...
    <section className="recap-themes-page" aria-labelledby="recap-themes-heading">
      <header className="recap-page-heading">
        <div>
          ...
          <h1 id="recap-themes-heading">Chọn cách kể lại ngày vui</h1>
          <p>
            Mỗi theme giữ nguyên album và lời chúc của bạn, chỉ thay đổi nhịp kể và không khí hình
            ảnh.
          </p>
        </div>
        {recap ? (
          <span className="recap-status-pill">
            <Check size={14} weight="bold" />{' '}
            {recap.status === 'PUBLISHED' ? 'Đã xuất bản' : 'Bản nháp'}
          </span>
        ) : null}
      </header>
      <div className="recap-library-toolbar">
        <div className="recap-library-filters">
          <label className="recap-library-search">
            ...
            <span className="sr-only">Tìm giao diện recap</span>
            <input
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Tìm theo tên, phong cách hoặc màu sắc"
            />
          </label>
          <label className="recap-style-filter">
        ...
        <span className="recap-library-count">{themes.length} giao diện khả dụng</span>
      </div>
      {loading ? (
        <div className="recap-library-grid" aria-label="Đang tải kho recap">
          {[1, 2].map((item) => (
            <div className="recap-library-skeleton" key={item}>
              <span />
              <i />
              <i />
            </div>
        ...
        <div className="recap-theme-coming recap-library-state">
          <WarningCircle size={30} />
          <strong>Chưa tải được kho recap</strong>
          <span>{error}</span>
        ...
        <div className="recap-library-grid">
          {visible.map((theme) => {
            ...
            return (
              <article
                className={`recap-library-card ${active ? 'is-active' : ''}`}
                key={theme.versionId}
              >
                <Artwork theme={theme} active={active} />
                <div className="recap-library-copy">
                  <div>
                    <h2>{theme.name}</h2>
                    <p>
                ...
                <footer>
                  ...
                  {active ? (
                    <AppLink className="button button-primary" to={studioRoutes.recap}>
                      <PencilSimple size={16} /> Chỉnh sửa recap
                    </AppLink>
                  ) : (
                    <button
                    ...
                    >
                      {saving === theme.versionId
                        ? 'Đang áp dụng…'
                        : recap
                          ? 'Dùng giao diện này'
                          : 'Bắt đầu với giao diện này'}
                    </button>
                  )}
                </footer>
              </article>
        ...
        <div className="recap-theme-coming recap-library-state">
          <ImagesSquare size={30} />
          <strong>Chưa có giao diện phù hợp</strong>
          <span>Thử đổi từ khóa tìm kiếm hoặc chờ thêm theme được phát hành.</span>
          <button
