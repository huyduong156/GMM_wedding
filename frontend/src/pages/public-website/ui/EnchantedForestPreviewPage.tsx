...
export function EnchantedForestPreviewPage(){
  ...
  useEffect(() => {
    ...
    const receive = (event: MessageEvent) => {
      ...
      else if (isLiveEditorScroll<EnchantedForestSectionConfig['order'][number]>(event.data)) document.querySelector(`[data-editor-section="${event.data.payload.sectionKey}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }
    window.addEventListener('message', receive)
    window.parent.postMessage({ type: liveEditorEvents.ready, version: 1 }, window.location.origin)
    return () => window.removeEventListener('message', receive)
  }, [editorMode])
  return <><EnchantedForestWebsite data={data} sectionConfig={sectionConfig} />{!editorMode ? <a className="ef-preview-exit" href="/studio/site/themes">Đóng xem trước</a> : null}</>
}

function readPreviewState(): PreviewState | null { try { return JSON.parse(sessionStorage.getItem('gmm-website-preview:enchanted-forest') ?? 'null') as PreviewState | null } catch { return null } }
