...
export function GreenHydrangeaPreviewPage() {
  ...
  useEffect(() => {
    ...
    const receive = (event: MessageEvent) => {
      ...
      } else if (isLiveEditorScroll<GreenHydrangeaSectionConfig['order'][number]>(event.data))
        document
  ...
  return (
    <>
      <GreenHydrangeaWebsite data={data} sectionConfig={sectionConfig} />
      {!editorMode ? (
        <a className="gh-preview-exit" href="/studio/site/themes">
          Đóng xem trước
        </a>
      ) : null}
    </>
...
function readPreviewState(): PreviewState | null {
  try {
    return JSON.parse(
      sessionStorage.getItem('gmm-website-preview:green-hydrangea') ?? 'null',
