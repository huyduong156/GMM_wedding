...
export function CherryBlossomGardenPreviewPage(){
  ...
  useEffect(() => {
    ...
    const receive = (event: MessageEvent) => {
      ...
      else if (isLiveEditorScroll<CherrySectionConfig['order'][number]>(event.data)) document.querySelector(`[data-editor-section="${event.data.payload.sectionKey}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }
    window.addEventListener('message', receive)
    window.parent.postMessage({ type: liveEditorEvents.ready, version: 1 }, window.location.origin)
    return () => window.removeEventListener('message', receive)
  }, [editorMode])
  return <><CherryBlossomGardenWebsite data={data} sectionConfig={sectionConfig} />{!editorMode ? <a className="cb-preview-exit" href="/studio/site/themes">Đóng xem trước</a> : null}</>
}

function readPreviewState(): PreviewState | null { try { return JSON.parse(sessionStorage.getItem('gmm-website-preview:cherry-blossom-garden') ?? 'null') as PreviewState | null } catch { return null } }
