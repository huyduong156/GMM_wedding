...
`Élan d’Amour` v1.1 là implementation đầu tiên theo contract này. Key kỹ thuật và route tương thích vẫn là `modern-luxe` và `/templates/invitations/modern-luxe/preview`. Mẫu dùng cover 5:7 chạm để mở, ba palette, thông tin hai gia đình, countdown, lịch trình, Google Maps, thêm lịch, slideshow, RSVP, sổ lưu bút và phần quà mừng. Những dữ liệu tương tác hiện là fixture/client prototype; API, moderation, token cá nhân và file ICS chưa được kết nối.

`Verdant Promise` v1.0 là implementation thứ hai, dùng art direction vườn kính nguyên bản, background và botanical frame do GMM tạo mới. Route preview là `/templates/invitations/verdant-promise/preview`. Mẫu giữ cùng section contract nhưng có motion riêng: lá rơi nhiều lớp, sương sáng, card mở 3D, frame thở và slideshow Ken Burns. Không sao chép asset, nội dung hoặc mã nguồn từ website tham khảo.

`Mây Hồng Có Đôi` v1.0 là implementation chibi storybook tại `/templates/invitations/chibi-daydream/preview`. Mẫu dùng coral, powder blue và ivory; popup phong bì 3D, ảnh cặp đôi/album do GMM tạo, vật phẩm từ `assets/icons/chibi`, calendar giấy, slideshow Ken Burns, RSVP, Maps, sổ lưu bút và QR minh họa. Chuyển động có reduced-motion fallback.
