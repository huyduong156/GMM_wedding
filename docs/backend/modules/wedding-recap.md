...
Khi publish, `WeddingRecap.slug` và `PublishedRecapSnapshot.slug` được đồng bộ từ `Wedding.slug`; script `npm run db:backfill-wedding-slugs` đồng bộ dữ liệu recap cũ. Migration `20260823194500_recap_api` bổ sung các JSONB field có default an toàn cho dữ liệu cũ. Unit tests nằm tại `src/modules/recaps/application/recap-service.test.ts`, bao phủ validation, revision, publish snapshot, public lookup và slug collision.
