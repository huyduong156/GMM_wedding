...
| `make sync` | Dừng Vite/Next dev server, cài dependency frontend/backend, start local dependencies including MinIO/bucket init, generate/validate Prisma và apply migration | Không | Đổi branch/máy sau khi pull code |
...
| `make stop-dev` | Dừng các process Vite/Next local theo command line | Không | Trước `npm ci` hoặc `make sync` |
