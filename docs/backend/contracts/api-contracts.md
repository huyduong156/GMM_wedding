...
- Workspace access: owner tạo/revoke link một lần qua `/weddings/{id}/workspace-access`; response tạo link chỉ trả raw `token` một lần, FE ghép URL theo frontend origin. User đã có tài khoản đăng nhập rồi claim qua `POST /workspace-access/{token}`. User mới gửi thêm `workspaceAccessToken` khi `POST /auth/register`: BE chỉ tạo claim chờ email verification; lúc `POST /auth/verify-email` thành công, transaction mới tạo `WeddingMember` ACTIVE và consume link. Register sau khi link đã dùng trả `WORKSPACE_ACCESS_ALREADY_USED`; response verify trả `workspaceAccessOutcome` (`JOINED`, `ALREADY_USED`, `REVOKED`, `EXPIRED`, `INVALID`) khi có claim để FE thông báo đúng trạng thái link. Role MVP gồm `OWNER`, `EDITOR`, `VIEWER`.
...
- Public: `GET /public/weddings/{weddingSlug}`, `GET /public/websites/{weddingSlug}`, `GET /public/invitations/{weddingSlug}/{guestSlug}`, `PUT .../{weddingSlug}/{guestSlug}/rsvp`, `POST .../{slug}/rsvps`, `POST|GET .../{slug}/wishes`. URL chung nhận `guestName`/tên khách; URL cá nhân resolve identity từ Guest slug và không yêu cầu nhập tên.
