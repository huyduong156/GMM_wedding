...
- Recap is the `RECAP` `WeddingContent` surface. Recap media and wishes reference that row, publish creates an immutable `PublishedRecapSnapshot`, and public lookup resolves through the single `Wedding.slug`.
