...
- Nhánh auth dùng thiết kế tại `docs/backend/contracts/authentication-implementation-design.md` và ADR 0006 (Proposed): opaque server session, Argon2id, exact-origin/Fetch Metadata/custom-header CSRF, Redis production rate limit và email qua adapter/outbox không lưu raw token. Core slice là register → verify-email → login → `/me` → logout; route vẫn Planned. Node 20 đã EOL nên nâng Node 24 và benchmark Argon2 Alpine là gate đầu tiên; MFA/step-up là production gate sau khi session core ổn định.
- Auth có hai login surface dùng chung identity/session: `/api/auth/login` cho owner và `/api/auth/admin/login` bắt buộc active `UserRole.ADMIN`; `/api/admin/me` cùng `requirePlatformAdmin()` là guard mẫu cho admin API. Local/test được phép `AUTH_RATE_LIMIT_DRIVER=disabled` qua `APP_ENV`, còn staging/production cấm bypass và production bắt buộc Redis.
- User login có recovery qua `/api/auth/forgot-password` và `/api/auth/reset-password`: acknowledgment không lộ account, token hash một lần TTL 30 phút, reset revoke toàn bộ session và audit. Account có active `UserRole.ADMIN` không nhận reset email từ flow public; admin recovery để dành cho quy trình vận hành/MFA.
- `/api/auth/resend-verification` hỗ trợ tài khoản pending với acknowledgment trung tính, rate limit theo IP/email, vô hiệu token email cũ và phát token TTL 24 giờ mới qua outbox/email adapter.
- Backend self-host Swagger UI tại `/api-docs` và OpenAPI YAML tại `/api/openapi`, cùng gate bởi server-side `API_DOCS_ENABLED`; local Compose bật mặc định, production không tự bật. External validator và “Try it out” được tắt.
- Identity model theo ADR 0007: owner và platform admin dùng chung canonical `User`/credential/session; quyền hệ thống chuyển từ scalar `User.platformRole` sang bảng ngắn gọn `UserRole` có grant/revoke/expiry. Wedding role tiếp tục ở `WeddingMember`; rank dùng `User.rankId -> Rank`, gói dùng `Plan`/`Subscription`, cả hai không cấp admin permission. Hai login surface khác UX/policy nhưng không tạo hai bảng account.
...
- Owner auth UI đã bao phủ `/register`, `/verify-email?token=...`, `/login`, `/forgot-password` và `/reset-password?token=...`; verify email dùng chung một request trong React Strict Mode để không consume token một lần hai lần. Resend verification UI/API client/cooldown đã chuẩn bị sau feature flag `VITE_AUTH_RESEND_ENABLED=false`, chỉ bật khi backend endpoint planned hoàn tất.
